#!/usr/bin/env python3
"""Inert public-source PG04 transport derivative; one MODEL test, never qualification."""
import argparse
import hashlib
import json
import os
...
REQUEST_SCHEMA = 'app29-pg04-public-v1'
REPOSITORY = 'kira-manga/Kira-backend'
BRANCH = 'remediation/app-29-pg04-public-validation-01'
SOURCE_HEAD = 'bffe162f36be666b19a07e6f993b5d2dea743635'
SOURCE_TREE = 'd20c606f9d0b34b37696806988c28c05b69453ed'
PUBLIC_PARENT = '3d839130c807f6a0a9b1c896c1c6cca4b41c4538'
SOURCE_PINS_SHA = '94cf49ef92cdd3097fb15d52294905bf8d5bd9d4739488ffd3e26d5fe00f810a'
PROFILE_ID = 'app-29-lease-dispatch-pg-profile-04'
PROFILE_FILE = 'profile/profile.json'
PROFILE_SHA = '0a07e8834d3b8d5f17dcc0eed167ef003cf5604d4737d28f04cd105b29fcf44e'
PROFILE_INIT = 'profile/profile.init.gradle'
PROFILE_INIT_SHA = 'd3198e1d96ec4fa4f79f4c878f2f3da54aff027421661c083b1b488891a473e2'
...
METHODS = {CREATOR_CLASS: ('MODEL work expiry inside actual afterJdbcCall cannot erase its admitted creator after epoch retirement',)}
...
CAPTURE_FOLDER = 'reports/lease-dispatch-pg-04'
...
def verify_source_profile(profile):
    require(profile['source_inventory'] == {
        ...
        'source_tree': SOURCE_TREE, 'source_parent': PUBLIC_PARENT, 'pinned_file_count': 82,
        'pins_sha256': SOURCE_PINS_SHA, 'expected_test_count': 1}, 'Wrong public source content binding')
    pins = profile['source_pins']
    require(len(pins) == 82 and all(isinstance(sha, str) and re.fullmatch('[0-9a-f]{64}', sha)
            and (name.startswith('src/') or name.startswith('vendor/pgjdbc-owned-cut/')) for name, sha in pins.items()),
            'Wrong reviewed82 source pin set')
    data = ''.join(name + '\t' + pins[name] + '\n' for name in sorted(pins)).encode('utf-8')
    require(hashlib.sha256(data).hexdigest() == SOURCE_PINS_SHA, 'Changed reviewed82 source pin map')
    return pins
...
def request():
    ...
    require(value['schema'] == REQUEST_SCHEMA and value['authorized'] is True
            and value['acquisition_authorization'] == 'PUBLIC_DECLARED_GRADLE_NATIVE05_W01_DOCKER_ONLY'
