...
def post_start_summary(obj, image_id, seed):
    ...
    return {'image_matches': obj.get('Image') == image_id, 'user_matches': config.get('User') == '10001:10001',
            ...
            'tutorial_seed_disabled': env.get('KIRA_TUTORIAL_SEED_ENABLED') == 'false',
...
def main():
    ...
    def remove_container(identity, graceful=False):
        ...
        if graceful:
            ...
            state = obj.get('State') if type(obj) is dict else None
            state = state if type(state) is dict else {}
            code, status = state.get('ExitCode'), state.get('Status')
            application_image = type(obj) is dict and obj.get('Image') == image_id
            report.setdefault('graceful_stop_states', {})[identity] = {
                'application_image': application_image,
                'running': state.get('Running') if type(state.get('Running')) is bool else None,
                'status': status if type(status) is str and status in ('created', 'running', 'paused', 'restarting', 'removing', 'exited', 'dead') else 'OTHER_OR_INVALID',
                'exit_code': code if type(code) is int and -1 <= code <= 255 else None,
                'oom_killed': state.get('OOMKilled') if type(state.get('OOMKilled')) is bool else None}
            check_owned('container', identity, obj)
            # Java's SIGTERM handler completes shutdown hooks then exits 128+15.
            need(state['Running'] is False and state['Status'] == 'exited' and state['OOMKilled'] is False
                 and type(code) is int and (code == 0 or (application_image and code == 143)))
    ...
    try:
        ...
        admin, jwt = secrets.token_hex(24), base64.b64encode(secrets.token_bytes(48)).decode('ascii')
        ...
        def migrate(role, jdbc, password, applied):
            ...
            matches = re.findall(rb'^Database migration complete; applied=([0-9]+) target=(13\.2|null)$', raw, re.M)
            expected_target = b'null' if applied == 0 else b'13.2'
            matched = matches == [(str(applied).encode(), expected_target)]
            report.setdefault('migration_summaries', {})[role] = {'matching_rows': len(matches), 'matches_expected': matched}
            need(matched)
        ...
        def application(role, jdbc, password, media_path, seed):
            values = {'SPRING_PROFILES_ACTIVE': 'prod', 'SPRING_DATASOURCE_URL': jdbc, 'SPRING_DATASOURCE_USERNAME': 'kira',
                      ...
                      'KIRA_COMPLETION_ENABLED': 'false', 'KIRA_TUTORIAL_SEED_ENABLED': 'false',
                      'KIRA_SIGNING_ACTIVE_KEY_ID': 'smoke', 'KIRA_SIGNING_PRIVATE_KEY': read(root / 'signing/smoke.private.b64').decode(),
            ...
            need(actual_env['KIRA_TUTORIAL_SEED_ENABLED'] == 'false')
