    ...
    fun `populated current and historical backups preserve exact bytes relationships and sequences`(version: Int) {
        withStartedBackupContainers(newComplaintPostgres("kira_source"), newComplaintPostgres("kira_restore")) { source, target ->
            ...
            source.connection().use { connection ->
                ...
                try {
                    if (version == 14) {
                        // This synthetic historical catalog has no bootstrap receipt; never mark it COMPLETE.
                        connection.exec("UPDATE document_publication_state SET bootstrap_phase='reconciliation_required' WHERE id=1")
                    }
            ...
            // Joined V13→V14 also applies the pinned credential and bootstrap migrations 13.1/13.2.
            assertEquals(if (version == 14) 0 else 3, flyway(target).migrate().migrationsExecuted)
