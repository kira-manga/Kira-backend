...
class OrdinaryComplaintTestReserveIT {
    ...
    @Test
    fun `independent desired D binds the run while P alone binds every counter and neither substitutes for the other`() = withFixture { f ->
        assertFalse(f.desired.configurationHashBytes().contentEquals(f.counters.syntheticPolicyDigest()))
        for (replaceRun in listOf(true, false)) {
            f.seed()
            if (replaceRun) {
                f.observer.update(
                    "UPDATE complaint_test_runs SET configuration_hash = ? WHERE data_scope_id = ?",
                    f.counters.syntheticPolicyDigest(),
                    f.scope.id,
                )
            } else {
                f.observer.update("UPDATE complaint_capacity_counters SET configuration_hash = ?", f.desired.configurationHashBytes())
            }
            val before = f.state()
    ...
    fun `missing counter catalogue or run and absent inconsistent or mismatched policy cannot spend`() = withFixture { f ->
        ...
        for (expected in listOf(null, ByteArray(32) { 9 })) {
            val before = f.state()
            assertRolledBack(f) { f.execute(port = JdbcComplaintTestReserveStore(f.jdbc, JdbcComplaintCapacityStore(f.jdbc, expected), f.desired)) }
            assertNoWrites(f)
            assertEquals(before, f.state())
        }
    ...
    fun `fabricated result prepared only early counter access and repeated use cannot complete the retained operation`() = withFixture { f ->
        ...
        assertRolledBack(f) {
            f.execute(
                port = port(f) { expected ->
                    ComplaintTestReserveSpendOperation.prepare(f.jdbc, expected, f.binding)
                    ComplaintTestReserveSpendResult.SPENT
                },
            )
        }
        assertRolledBack(f) {
            f.execute(
                port = port(f) { expected ->
                    val operation = ComplaintTestReserveSpendOperation.prepare(f.jdbc, expected, f.binding)
    ...
    fun `stale and cross thread use poison the attempted owner without undoing an earlier committed allocation`() = withFixture { f ->
        ...
        assertEquals(
            ...
            f.execute(
                port = port(f) { expected ->
                    old = ComplaintTestReserveSpendOperation.prepare(f.jdbc, expected, f.binding)
    ...
    fun `concurrent same expected slice waits on counters and only one spends while the stale caller rolls back`() = withFixture(maximumPoolSize = 3) { f ->
        ...
        val stores = listOf(
            f.store,
            JdbcComplaintTestReserveStore(secondJdbc, JdbcComplaintCapacityStore(secondJdbc, f.counters.syntheticPolicyDigest()), f.desired),
        )
