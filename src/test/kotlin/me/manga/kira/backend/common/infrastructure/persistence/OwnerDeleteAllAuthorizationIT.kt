...
class OwnerDeleteAllAuthorizationIT {
    ...
    fun `empty and hundred-target authorization commits one immutable paid snapshot and releases work only after actual cleanup`() {
        for (count in listOf(0, 100)) withFixture { f ->
            ...
            f.afterStep = { step ->
                if (step == DeleteAllStep.AUDIT) {
                    ...
                    assertEquals(true, f.observer.queryForObject(
                        ...
                        Boolean::class.java, held.identity.first,
                    ))
    ...
    fun `competing exact active observations serialize into one charge and late mismatching or multiple receipts cannot be adopted`() {
        ...
        for (kind in listOf("IN_PROGRESS", "WRONG_KEY", "MULTIPLE")) withFixture { f ->
            val candidate = f.enrolled()
            f.admitted(candidate) { observed, admission ->
    ...
    fun `unrelated PREPARED and VERIFIED work globally blocks new snapshots but exact reload neither rescans nor rewrites`() = withFixture { f ->
        ...
        for (verified in listOf(false, true)) {
            ...
            if (verified) assertInstanceOf(CommittedOwnerDeleteAllWork.RecordedVerified::class.java, replay)
            else assertInstanceOf(CommittedOwnerDeleteAllWork.Prepared::class.java, replay)
        ...
        assertEquals(1, f.observer.update(
            "UPDATE complaint_journal_control SET publication_epoch = 12, maintenance_closed = true, " +
                "checkpoint_started_at = clock_timestamp() - interval '2 days', checkpoint_completed_at = clock_timestamp() - interval '1 day' " +
                "WHERE data_scope_id = ?", ComplaintDataScope.LIVE.id,
        ))
    ...
    @Test
    fun `exact reload rejects byte-equivalent noncanonical storage wrong column tuple and sealed PREPARED epoch without replacing the old route`() = withFixture { f ->
        val candidate = f.enrolled()
        ...
        f.observer.update("UPDATE complaint_journal_publications SET event_bytes = ?, semantic_hash = ? WHERE event_id = ?", changed, ownerDeleteAllTestDigest(changed), event.route.eventId)
        var before = f.state()
        assertRolledBack(assertThrows { f.prepared(candidate) })
        assertEquals(before, f.state())
        f.observer.update("UPDATE complaint_journal_publications SET event_bytes = ?, semantic_hash = ? WHERE event_id = ?", bytes, ownerDeleteAllTestDigest(bytes), event.route.eventId)
        ...
        val control = f.controlRow()
        f.observer.update("UPDATE complaint_journal_control SET publication_epoch = 12, seal_epoch = 11 WHERE data_scope_id = ?", ComplaintDataScope.LIVE.id)
        before = f.state()
        assertRolledBack(assertThrows { f.prepared(candidate) })
        assertEquals(before, f.state())
        f.restoreControl(control)
    ...
    fun `native commit rejection committed completion failure server loss and cancellation never release work from the failed attempt`() {
        for (mode in listOf("COMMIT", "TAIL", "SERVER_LOSS", "INTERRUPT")) withFixture { f ->
            val candidate = f.enrolled()
            val before = f.state()
            val counters = f.counters()
            var operation: ComplaintOwnerDeleteAllOperation? = null
    ...
    fun `installation lock linearizes a prior synthetic LIVE create and forces a later ACTIVE version recheck to refuse`() {
        for (createFirst in listOf(true, false)) withFixture { f ->
            val candidate = f.enrolled()
    ...
    private fun insertInProgress(f: OwnerDeleteAllAuthorizationFixture, candidate: InstallationDeletionCandidate, key: UUID) {
        assertEquals(1, f.observer.update(
            ...
            candidate.installation.id, key, candidate.credentialVersion, ComplaintDeleteAllFingerprint.of(candidate).bytes(), ComplaintDataScope.LIVE.id,
        ))
    }
    ...
    private fun assertPersisted(f: OwnerDeleteAllAuthorizationFixture, candidate: InstallationDeletionCandidate, event: OwnerDeleteAllJournalEventV1, count: Int) {
        val row = f.observer.queryForMap(
            "SELECT d.*, p.event_bytes, p.semantic_hash, p.target_count, p.state AS publication_state, p.object_version, " +
                ...
                "WHERE d.installation_id = ?", candidate.installation.id,
        )
        ...
        assertEquals(true, f.observer.queryForObject(
            ...
            Boolean::class.java, OwnerDeleteAllCapacityCharges.RECOVERY.toLongArray().joinToString(",", "{", "}"), event.route.eventId,
        ))
        val audit = f.observer.queryForMap(
            "SELECT a.* FROM audit_log a JOIN installation_deletion_receipts d ON a.created_at = d.authorized_at " +
                "WHERE d.installation_id = ? AND a.action = 'COMPLAINT_INSTALLATION_DELETE_AUTHORIZED'", candidate.installation.id,
        )
    ...
    private fun markVerified(f: OwnerDeleteAllAuthorizationFixture, event: String) {
        ...
        assertEquals(1, f.observer.update(
            "UPDATE complaint_journal_publications SET state = 'VERIFIED', object_version = 'synthetic-object-v1', ciphertext_hash = ?, " +
                ...
                "verification_bytes = ?, verification_hash = ? WHERE event_id = ?", ownerDeleteAllTestDigest(bytes), bytes, ownerDeleteAllTestDigest(bytes), event,
        ))
    }
    ...
    private fun restoreCounters(f: OwnerDeleteAllAuthorizationFixture, rows: String) = f.transaction { selected ->
        ...
        assertEquals(22, selected.update("INSERT INTO complaint_capacity_counters SELECT * FROM jsonb_populate_recordset(NULL::complaint_capacity_counters, ?::jsonb)", rows))
    }

    private fun requiredStorage(): Long = (OwnerDeleteAllCapacityCharges.AUTHORIZATION + OwnerDeleteAllCapacityCharges.RECOVERY)[ComplaintCapacityCounter.STORAGE_BYTES]
