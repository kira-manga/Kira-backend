...
internal object TestOrdinarySealCasesV1 {
    ...
    fun completeHistory(tls: VersionBoundPersistenceConnectedFixture, syntheticExpiredReceipt: Boolean, recoveryReserved: Boolean = false) = withOrdinarySealRun(
        tls, histories = if (recoveryReserved) 1 else 2, drain = !recoveryReserved, recoverMissingInstallation = recoveryReserved,
    ) { f, history ->
        val h = checkNotNull(history)
        if (recoveryReserved) {
            assertEquals("RECOVERY_RESERVED", f.observer.queryForObject("SELECT state FROM complaint_installation_ids WHERE id = ?", String::class.java, h.actor.id))
            assertEquals(0L, f.observer.queryForObject("SELECT count(*) FROM app_installations WHERE id = ?", Long::class.java, h.actor.id))
            assertEquals("PARTIAL", f.observer.queryForObject("SELECT state FROM complaint_recovery_capacity_reservations WHERE event_id = ?", String::class.java, h.eventId))
        }
        ...
        assertEquals(TestRunOrdinarySealResultV1.CAPTURED_LOCAL_ORDINARY_SET_SEAL_VERIFIED, original.seal())
        f.assertReleased(original)
        assertVerifiedLocalOnly(f, cutoff = 11, expectedCount = h.histories.size.toLong())
        assertInstallationObservation(f, original, listOf(h.actor.id), recoveryReserved)
    ...
    fun invalidInstallationSource(tls: VersionBoundPersistenceConnectedFixture, cut: TestOrdinarySealInstallationCutV1) =
        withOrdinarySealRun(tls, histories = 1,
            expireClosedSetupPredecessors = cut !in setOf(TestOrdinarySealInstallationCutV1.FENCE, TestOrdinarySealInstallationCutV1.DEADLINE),
        ) { f, history ->
    ...
    internal fun assertInstallationObservation(f: TestOrdinarySealObservationV1, original: TestRunOrdinarySealV1, expectedIds: List<UUID>, recoveryReserved: Boolean = false) {
        ...
        val rows = f.observer.query("SELECT i.id, i.state, c.state AS credential_state FROM complaint_installation_ids i " +
            ...
            assertEquals(if (recoveryReserved) "RECOVERY_RESERVED" else "ACTIVE", row.getString("state"))
            assertEquals(if (recoveryReserved) null else "ACTIVE", row.getString("credential_state"))
