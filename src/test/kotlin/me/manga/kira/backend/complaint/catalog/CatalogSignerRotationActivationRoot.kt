...
/** Named TARGET-only activation3 assembly on the existing real TLS carrier. No inherited campaign or caller-supplied SQL/cloud evidence. */
internal class CatalogSignerRotationActivationRoot(private val f: CatalogSignerRotationActivationFixture) : AutoCloseable {
    ...
    private val attempts = mutableListOf<CatalogSignerRotationActivationV1>()
    ...
    fun prepare(
        ...
        assembly.assembleTargetSignerRotationActivation(
            inputs,
        ...
        assertNotSame(f.projectedRoot.process, process)
        assertNotSame(f.projectedRoot.process.pools, process.pools)
        ...
        assertNotSame(f.projectedRoot.process.consumers, process.consumers)
        assertNotSame(f.projectedRoot.process.catalogReadback, process.catalogReadback)
        assertNotSame(f.projectedRoot.process.catalogSignerRotation, process.catalogSignerRotation)
        if (sameD) {
            assertArrayEquals(f.delivery.selectedCanonical, process.canonicalBytes())
        ...
        assertPurposeClosed()
        jdbc = CatalogSignerRotationProbeJdbc(coordinator, observeActivationQueries = true)
        jdbc.beforeSql = {
            assertEquals(f.signing.createdClients, f.signing.closedClients)
        ...
        // Same concrete executors/source/manager, installed before ANY activation owner or full-B binding can retain identity.
        ...
        install(
            "signerRotationActivationExecutor",
            ComplaintCatalogSignerRotationActivationPhaseExecutorV1(coordinator, jdbc),
        )
        assembly.prepareTargetSignerRotationActivation(PersistenceTimeBudget.start(30_000, clock))
        assertEquals(PersistenceLifecycleObservation.READY, coordinator.observePreparation())
        assertPurposeClosed()
        assertTrue(jdbc.calls.isEmpty())
    }

    fun begin(): CatalogSignerRotationActivationV1 = CatalogSignerRotationActivationV1.withHttpFixture(
        process,
        f.signing::httpClient,
        f.http::putClient,
        f.http::readClient,
    ...
    fun recover(
        ...
        try {
            ...
            assertEquals(signClients, f.signing.createdClients)
    ...
    fun continueUnattempted(original: CatalogSignerRotationActivationV1, previous: CatalogSignerRotationActivationV1) = try {
        original.continueUnattempted(
            f.request,
            previous,
            CatalogSignerRotationActivationFixture.SIGNING_CREDENTIALS,
            CatalogGenesisPublishHttpFixture.PUT_CREDENTIALS,
    ...
    fun project(original: CatalogSignerRotationActivationV1) = try {
        original.project()
    } finally {
        assertNoLostAssertions()
    }
    ...
    fun assertReleased(original: CatalogSignerRotationActivationV1, reserved: Boolean = true) {
        original.requireActualCleanup()
        assertEquals(reserved, poolTestField<Boolean>(original, "reserved"))
    ...
    fun assertPurposeClosed() {
        val owner = poolTestField<PersistenceJdbcLifecycleOwner>(assembly, "targetOwner")
        val original = active()
        assertTrue(owner.catalogSignerRotationActivation && coordinator.catalogSignerRotationActivation)
        assertFalse(
            owner.catalogSignerRotationDelivery || owner.catalogSignerRotationRecovery || owner.catalogSignerRotationAuthoring ||
                owner.catalogGenesisAuthoring ||
    ...
    private fun assertOriginalFilesClosed(original: CatalogSignerRotationActivationV1) {
    ...
    private fun assertTransportDisposed() {
        assertEquals(f.signing.createdClients, f.signing.closedClients)
        assertEquals(f.signing.createdClients, f.signing.returnedClientCloses)
        f.signing.replies.forEach {
