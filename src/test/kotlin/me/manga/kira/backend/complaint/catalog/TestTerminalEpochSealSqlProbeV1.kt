...
/** Observes the real fixed holder and SQL; cannot provide a result, transaction outcome or proof. */
internal class TestTerminalEpochSealSqlProbeV1(private val f: TestRunPurgeFixtureV1) :
    JdbcTemplate(f.registration.process.pools.catalogCoordinator.dataSource), AutoCloseable {
    var original: TestRunTerminalEpochSealV1? = null
    ...
    private val owners = linkedMapOf<PersistencePhaseContext, TestRunTerminalEpochSealV1>()
    private val assertion = AtomicReference<AssertionError?>()
    private val executor = f.registration.process.pools.catalogCoordinator.testTerminalEpochSeal
    ...
    private fun <T> observed(sql: String, args: Array<out Any?>, action: () -> T): T = try {
        ...
        val owner = ownedCutField(phase, "testTerminalEpochSeal") as TestRunTerminalEpochSealV1
        ...
        assertEquals(PersistencePhasePath.COMPLAINT_TEST_TERMINAL_EPOCH_SEAL, path)
        assertEquals(sql.count { it == '?' }, args.size)
        if (owner.step in setOf(TestTerminalEpochSealStepV1.VERIFY, TestTerminalEpochSealStepV1.COMPLETE)) {
            f.sealHttp.assertDisposed()
            assertEquals(0L, f.registration.process.publicationLanes.activeOwners().totalOwners,
                "Actual native return and shared-J retirement precede even VERIFY authentication.")
        }
        ...
        assertTrue(f.p.advisory(connection, "complaint-journal-epoch", "ExclusiveLock"), "VERIFY is not the purge's receiptless path.")
    ...
    fun assertReleased(requireCommitted: Boolean = true) {
        ...
        observations.forEach { (phase, value) ->
            ...
            assertTrue(phase.testTerminalEpochSealResourcesRetired(owners.getValue(phase)))
            if (requireCommitted) {
                assertTrue(phase.testTerminalEpochSealCleanupProven(owners.getValue(phase)))
    ...
    class Call(val phase: PersistencePhaseContext, val step: TestTerminalEpochSealStepV1, val sql: String, val arguments: List<Any?>)
