...
internal class CatalogGenesisFinalizationCases(
    ...
    private val digest = ByteArray(32) { 57 } // Synthetic independent capacity binding, not authenticated deployment configuration.
    ...
    fun completeProjectReplay() {
        ...
        val port = provider()
        val result = executor.resumeGenesis(port, fixture.initial, fixture.current, fixture.policy(), digest)
        ...
        jdbc.steps.clear()
        val replay = executor.resumeGenesis(provider(), fixture.initial, fixture.current, later, digest)
    ...
    fun pendingResumeAndHistory() {
        stageSigned()
        val verified = proof()
        assertEquals(CatalogGenesisFinalizationState.COMPLETED_PENDING, executor.completeGenesis(verified, digest).state)
        val pending =
        ...
        val conflict = state()
        jdbc.steps.clear()
        rolledBack { executor.projectGenesis(verified, digest) }
        ...
        assertEquals(
            ...
            executor.resumeGenesis(provider(), fixture.initial, fixture.current, fixture.policy(), digest).state,
        )
        released()
    }
    ...
    fun atomicFailures() {
        ...
        for (step in listOf("complete", "head", "readback")) {
            ...
            rolledBack { executor.completeGenesis(verified, digest) }
            assertEquals(prepared, state())
        }
        deferredCommitFailure("head") { executor.completeGenesis(verified, digest) }
        assertEquals(prepared, state())
        jdbc.afterSql = null
        executor.completeGenesis(verified, digest)
        val pending = state()
        ...
        deferredCommitFailure("initial") { executor.projectGenesis(verified, digest) }
        assertEquals(pending, state())
        jdbc.afterSql = null
        released()
    }
    ...
    fun sealedAndProjectionTail() {
        stageSigned()
        val input = CatalogGenesisMutationInput.complete(proof())
        ...
        val failed = assertThrows(PersistencePhaseException::class.java) {
            executor.resumeGenesis(provider(), fixture.initial, fixture.current, fixture.policy(), digest)
        }
    ...
    fun frozenPreimagesAndInitialBindings() {
        ...
        val empty = state()
        rolledBack { executor.completeGenesis(supplied, digest) }
        ...
        val prepared = state()
        rolledBack { executor.completeGenesis(wrongToken, digest) }
        assertEquals(prepared, state())
        ...
        observer.update(
            ...
            HexFormat.of().parseHex(manifest.initialWriterRegistry.eventWriter.liveRange.configurationSha256),
            LIVE,
        )
        executor.completeGenesis(verified, digest)
        executor.projectGenesis(verified, digest)
        ...
        rolledBack { executor.projectGenesis(replay, digest) }
    ...
    fun coldRefusal() {
        ...
        assertThrows(CatalogReadbackException::class.java) {
            executor.resumeGenesis(port, fixture.initial, fixture.current, fixture.policy(), ByteArray(31))
        }
        ...
        assertThrows(OfflineTrustBundleException::class.java) {
            executor.resumeGenesis(port, fixture.initial, forged, fixture.policy(), digest)
        }
