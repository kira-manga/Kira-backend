...
class OwnerDeleteAllJournalPublisherV1IT {
    ...
    fun `conflict and lost responses reconcile finitely with one byte identical candidate retry`() = withPrepared { fixture, work ->
        ...
        for (mode in listOf("BOTH_LOST", "ACK_EMPTY", "PRECONDITION_EMPTY", "LIST_403", "GET_404")) {
            ...
            fixture.publisher().use { publisher ->
                ...
                assertEquals(if (mode == "BOTH_LOST") 2 else if (mode == "LIST_403") 0 else 1, fixture.requests.count { it.kind == "PUT" })
    ...
    fun `cheap version proof precedes unwrap and real authentication rejects conflicting content`() = withPrepared { fixture, work ->
        ...
        for (mode in listOf("SHORT", "EXPIRED", "FUTURE", "REQUEST_NOT_AFTER_CREATION")) {
            ...
            fixture.stored = original.copy(
                ...
                retainUntil = retain,
                metadata = original.metadata + ("kira-journal-retain-until" to if (mode == "REQUEST_NOT_AFTER_CREATION") {
                    original.lastModified.toString()
                } else retain.toString()),
            )
            val before = fixture.decrypted()
    ...
    fun `raw body header and XML limits reject one over before any unsafe decode or fallback`() = withPrepared { fixture, work ->
        ...
        for ((name, value) in listOf(
            "Location" to "https://different.invalid/", "Content-Encoding" to "gzip", "Content-Range" to "bytes 0-1/2",
            "Transfer-Encoding" to "chunked", "x-amz-trailer" to "x-amz-checksum-sha256",
        )) assertListReply(fixture, work, xmlReply(empty).apply { header(this, name, value) }, false)
        assertListReply(fixture, work, xmlReply(empty).apply { header(this, "Content-Length", null); header(this, "Transfer-Encoding", "gzip") }, false)
        assertListReply(fixture, work, xmlReply(empty).apply { header(this, "Content-Length", null); header(this, "Transfer-Encoding", "chunked") }, true)
    ...
    fun `one shared shrinking attempt and retained cleanup refuse signals races and late success`() = withPrepared { fixture, work ->
        for (millis in listOf(800L, 900L)) {
            ...
            fixture.respond = { request ->
                fixture.statefulReply(request).apply { beforeCall = { calls.add(request.kind); fixture.nanos += millis * 1_000_000 } }
            }
            ...
            fixture.publisher().use { publisher ->
                if (millis == 800L) assertEvidence(fixture, publisher.publish(work)) else {
    ...
    private fun assertRealCrypto(
        ...
        for (mode in listOf("OTHER_TARGETS", "WRONG_TUPLE", "TAMPERED_TAG")) {
            ...
            fixture.publisher().use { publisher ->
                ...
                assertRedacted(failure)
                when (mode) {
                    "OTHER_TARGETS" -> assertEquals(JournalPublicationFailureV1.CONFLICT, assertInstanceOf(JournalPublicationExceptionV1::class.java, failure).code)
    ...
    private fun assertListReply(
        ...
        fixture.publisher().use { publisher ->
            if (accepted) assertEvidence(fixture, publisher.publish(work)) else {
    ...
    private fun assertEnvelopeLimit(fixture: OwnerDeleteAllJournalPublisherFixture, work: CommittedOwnerDeleteAllWork.Prepared) {
    ...
    private fun assertSignals(fixture: OwnerDeleteAllJournalPublisherFixture, work: CommittedOwnerDeleteAllWork.Prepared) {
        ...
        for (stage in listOf("PREPARE", "CALL")) {
            for ((expected, signal) in signals) {
                fixture.reset()
                if (stage == "PREPARE") fixture.beforePrepare = { throw signal() } else {
    ...
    private fun assertCleanupCustody(fixture: OwnerDeleteAllJournalPublisherFixture, work: CommittedOwnerDeleteAllWork.Prepared) {
        for (mode in listOf("ABORT", "CLOSE", "FATAL_OVER_CANCELLATION", "WRAPPED_FATAL_OVER_CANCELLATION")) {
            ...
            try {
                val failure = checkNotNull(runCatching { publisher.publish(work) }.exceptionOrNull())
                assertRedacted(failure)
                if (mode.contains("FATAL")) assertInstanceOf(JournalPublicationFatalV1::class.java, failure) else {
    ...
    private fun assertLateNativeClose(fixture: OwnerDeleteAllJournalPublisherFixture, work: CommittedOwnerDeleteAllWork.Prepared) {
        fixture.reset()
        fixture.stored = fixture.objectFor(fixture.envelope())
        val before = fixture.decrypted()
        val publisher = fixture.publisher()
    ...
    private fun scalar(document: String, name: String, value: String): String = document.replace(Regex("<$name>[^<]*</$name>"), "<$name>$value</$name>")

    private fun header(reply: S3CatalogReply, name: String, value: String?) {
    ...
    private fun httpDate(value: Instant): String = DateTimeFormatter.RFC_1123_DATE_TIME.format(value.atZone(ZoneOffset.UTC))
    ...
    private fun withPrepared(
        count: Int = 1,
        test: (OwnerDeleteAllJournalPublisherFixture, CommittedOwnerDeleteAllWork.Prepared) -> Unit,
    ) = withFixture { auth ->
        val candidate = auth.enrolled()
    ...
    private fun assertNoRequests(fixture: OwnerDeleteAllJournalPublisherFixture) {
        assertTrue(fixture.requests.isEmpty())
        assertTrue(fixture.kms.requests.isEmpty())
    }

    private fun assertClientsClosed(fixture: OwnerDeleteAllJournalPublisherFixture) {
        fixture.assertClosedExchanges()
    ...
    private fun assertEvidence(fixture: OwnerDeleteAllJournalPublisherFixture, readback: OwnerDeleteAllJournalReadbackV1) {
    ...
    private fun assertWire(fixture: OwnerDeleteAllJournalPublisherFixture) {
