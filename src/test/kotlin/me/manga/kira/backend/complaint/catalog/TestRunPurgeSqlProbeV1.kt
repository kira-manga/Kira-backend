...
internal class TestRunPurgeSqlProbeV1(private val f: TestRunPurgeFixtureV1) :
    JdbcTemplate(f.registration.process.pools.catalogCoordinator.dataSource), AutoCloseable {
    var original: TestRunPurgePublicationV1? = null
    ...
    private val owners = linkedMapOf<PersistencePhaseContext, TestRunPurgePublicationV1>()
    private val assertion = AtomicReference<AssertionError?>()
    private val executor = f.registration.process.pools.catalogCoordinator.testRunPurge
    ...
    private fun <T> observed(sql: String, args: Array<out Any?>, action: () -> T): T = try {
        ...
        val owner = ownedCutField(phase, "testRunPurge") as TestRunPurgePublicationV1
        ...
        val verify = owner.step === TestRunPurgeStepV1.VERIFY
        assertEquals(if (verify) PersistencePhasePath.COMPLAINT_TEST_RUN_PURGE_VERIFY
            else PersistencePhasePath.COMPLAINT_TEST_RUN_PURGE_PUBLICATION, path)
        assertEquals(sql.count { it == '?' }, args.size)
        if (verify) {
            assertTrue(sql in setOf(AUTHENTICATE, "SELECT clock_timestamp()", TestRunPurgeSqlV1.publication,
                TestRunPurgeSqlV1.verify), "Receiptless VERIFY must touch only its publication, not sidecar/control/run/receipt/counters.")
        ...
        assertEquals(!verify, f.p.advisory(connection, "complaint-maintenance-v1", "ShareLock"))
        assertEquals(!verify, f.p.advisory(connection, "complaint-journal-epoch", "ExclusiveLock"))
    ...
    fun assertReleased(requireCommitted: Boolean = true) {
        ...
        observations.forEach { (phase, value) ->
            ...
            assertTrue(phase.testRunPurgeResourcesRetired(owners.getValue(phase)))
            if (requireCommitted) {
                assertTrue(phase.testRunPurgeCleanupProven(owners.getValue(phase)))
    ...
    class Call(val phase: PersistencePhaseContext, val step: TestRunPurgeStepV1,
