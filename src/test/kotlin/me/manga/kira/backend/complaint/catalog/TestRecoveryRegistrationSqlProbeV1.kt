 ...
 * actual transaction faults in cases below, never an alternate production operation.
...
internal class TestRecoveryRegistrationSqlProbeV1(
    ...
    var after: (TestRecoveryRegistrationSqlCallV1) -> Unit = {}
    ...
    private fun unexpectedWrite(): Nothing {
        val failure = AssertionError("Recovery registration must not execute UPDATE/INSERT/DELETE/DDL.")
    ...
    private fun <T> observed(sql: String, args: Array<out Any?>, action: () -> T): T = try {
        ...
        assertEquals(PersistencePhasePath.COMPLAINT_TEST_NAMESPACE_RECOVERY_REGISTRATION, ownedCutField(phase, "path"))
        assertSame(checkNotNull(original), ownedCutField(phase, "testRecoveryRegistration"))
        ...
        val call = TestRecoveryRegistrationSqlCallV1(phase, sql, args.map { if (it is ByteArray) it.copyOf() else it })
        calls.add(call)
    ...
    fun assertCommittedAndReleased() {
        ...
        observations.keys.forEach { phase ->
            ...
            assertTrue(phase.testRecoveryRegistrationCleanupProven(checkNotNull(original)))
        }
    }
...
internal class TestRecoveryRegistrationSqlCallV1(val phase: PersistencePhaseContext, val sql: String, val arguments: List<Any?>)
