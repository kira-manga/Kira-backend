 ...
 *  - (a) migrate V1..V3 only, then migrate to latest → V4..V14 apply on top;
 *  - (b) migrate V1..V4 only, then migrate to latest → V5..V14 apply.
 *
...
class FlywayIncrementalOrderIT {
    ...
    @Test
    fun `V3 baseline then upgrade to latest applies V4 through V14 in order`() {
        withFreshDatabase("flyway_order_v3_baseline") { database ->
            ...
            assertEquals(
                ...
                "V4..V14 apply on top, in order",
            )
        }
    }

    @Test
    fun `V4 baseline then upgrade to latest applies V5 through V14`() {
