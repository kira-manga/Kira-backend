-- Narrow forward change: retain the original consumed grant on rejected ADMIN_BATCH_DELETE receipts.
-- All other result branches are V24 unchanged; external/APPLIED checks already admit this family.
...
ALTER TABLE complaint_idempotency_receipts ADD CONSTRAINT chk_complaint_receipt_result CHECK ((
    ...
    OR (outcome = 'REJECTED' AND ack_ids IS NULL AND ack_versions IS NULL
        ...
        AND (consumed_grant_id IS NULL OR (actor_kind = 'ADMIN'
            AND operation IN ('ADMIN_EDIT', 'ADMIN_STATUS', 'ADMIN_CLOSURE', 'ADMIN_DELETE', 'ADMIN_BATCH_STATUS', 'ADMIN_BATCH_DELETE') AND complaint_is_v4(consumed_grant_id)))
