-- One narrow forward association: ADMIN_BATCH_STATUS applied/rejected receipts retain the original grant UUID.
-- Existing NULL is unknown and remains replayable. No FK, backfill, deletion widening or retention change.
-- The other result/external predicates are the current V23/V22 definitions unchanged.
...
ALTER TABLE complaint_idempotency_receipts ADD CONSTRAINT chk_complaint_receipt_result CHECK ((
    ...
    OR (outcome = 'REJECTED' AND ack_ids IS NULL AND ack_versions IS NULL
        ...
        AND (consumed_grant_id IS NULL OR (actor_kind = 'ADMIN'
            AND operation IN ('ADMIN_EDIT', 'ADMIN_STATUS', 'ADMIN_CLOSURE', 'ADMIN_DELETE', 'ADMIN_BATCH_STATUS') AND complaint_is_v4(consumed_grant_id)))
...
ALTER TABLE complaint_idempotency_receipts ADD CONSTRAINT chk_complaint_receipt_external CHECK ((
    ...
    OR (operation NOT IN ('OWNER_DELETE', 'ADMIN_DELETE', 'ADMIN_BATCH_DELETE')
        ...
        AND (consumed_grant_id IS NULL OR (actor_kind = 'ADMIN'
            AND operation IN ('ADMIN_EDIT', 'ADMIN_STATUS', 'ADMIN_CLOSURE', 'ADMIN_BATCH_STATUS') AND complaint_is_v4(consumed_grant_id)))
