-- Forward-only OWNER_CREATE completion: globally occupied UUIDs have one generic terminal result.
-- V14 remains byte-identical. All prior result branches and separate phase/expiry/external checks remain.
...
ALTER TABLE complaint_idempotency_receipts ADD CONSTRAINT chk_complaint_receipt_result CHECK ((
    ...
    OR (outcome = 'REJECTED' AND ack_ids IS NULL AND ack_versions IS NULL
        ...
        AND ((problem_code IN ('COMPLAINT_NOT_FOUND', 'COMPLAINT_PARENT_NOT_FOUND') AND response_status = 404)
            ...
            OR (operation = 'OWNER_CREATE' AND problem_code = 'COMPLAINT_RESOURCE_ID_REUSED' AND response_status = 409)))
