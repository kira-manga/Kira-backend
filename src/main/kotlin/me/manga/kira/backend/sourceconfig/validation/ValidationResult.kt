 ...
 */
data class ValidationWarning(val code: String, val path: String, val message: String)
...
data class ValidationResult(val isValid: Boolean, val errors: List<ValidationError>, val warnings: List<ValidationWarning> = emptyList()) {
    companion object {
        fun of(errors: List<ValidationError>, warnings: List<ValidationWarning> = emptyList()): ValidationResult =
