...
object StructuralAuthoringGate {
    ...
    fun check(
        ...
        checkApiIdentifier(source.api)

        // (e) FIELD_TOO_LONG — identity/denormalized values over their DB column limits.
        fieldTooLong("displayName", source.displayName.length, DISPLAY_NAME_MAX)
        fieldTooLong("language", source.language.length, LANGUAGE_MAX)
        fieldTooLong("engine", source.engine.length, ENGINE_MAX)
        fieldTooLong("baseUrl", source.baseUrl.length, BASE_URL_MAX)
    }

    private fun checkApiIdentifier(api: String) {
        val invalid =
            api.isBlank() ||
                api.length > API_MAX ||
                api != api.trim() ||
    ...
    private fun fieldTooLong(
        ...
        if (length > max) {
            throw BadRequestException(
                "$field exceeds its maximum length of $max characters.",
                code = FIELD_TOO_LONG,
            )
        }
    }
