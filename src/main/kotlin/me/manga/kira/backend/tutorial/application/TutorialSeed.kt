...
class TutorialSeedService(
    ...
    private fun import(resource: String) = ClassPathResource(resource).inputStream.use {
        media.importSeedAsset(it.readNBytes(TutorialMediaService.MAX_BYTES + 1)).id
    }
...
class TutorialStartupValidator(private val repository: TutorialRepository, private val reconciliation: TutorialMediaReconciliationService) :
    ApplicationRunner {
    override fun run(args: ApplicationArguments) {
        // Always report-only, including when the separately scheduled mutation worker is enabled.
        val report = reconciliation.inspect()
        if (!report.clean || report.retainedQuarantineFiles > 0) {
            log.warn(
                "Tutorial media inspection (rows={}, publishedRows={}, publishedVerified={}, complete={}, findings={}, retainedQuarantine={})",
                report.rowsChecked,
                report.publishedRowsChecked,
                report.publishedComplete,
                report.complete,
                report.findings.groupingBy { it.issue }.eachCount(),
                report.retainedQuarantineFiles,
            )
        }
        check(report.publishedComplete) {
            val missing = report.findings.firstOrNull { it.published == true && it.issue == MediaStorageIssue.MISSING }
            if (missing != null) {
                "published tutorial media file is missing: ${missing.mediaId}"
            } else {
                "published tutorial media verification is incomplete or corrupt"
            }
        }
        check(repository.countInvalidPublishedTutorialReferences() == 0) { "invalid published tutorial/category/media references detected" }
    }

    companion object {
        private val log = LoggerFactory.getLogger(TutorialStartupValidator::class.java)
    }
}
