...
internal object ComplaintAdminBatchParser {
    ...
    fun parse(bytes: ByteArray, scope: ComplaintDataScope, key: String): ComplaintAdminBatchInput = parse(bytes, scope, key, Accepted.BOTH)
    fun parseStatus(bytes: ByteArray, scope: ComplaintDataScope, key: String): ComplaintAdminBatchStatusInput =
        (parse(bytes, scope, key, Accepted.STATUS_ONLY) as ComplaintAdminBatchInput.Status).input

    private enum class Accepted { STATUS_ONLY, BOTH }
    @Suppress("SwallowedException")
    private fun parse(bytes: ByteArray, scope: ComplaintDataScope, key: String, accepted: Accepted): ComplaintAdminBatchInput {
    ...
    private fun read(json: JsonParser, scope: ComplaintDataScope, key: String, accepted: Accepted): ComplaintAdminBatchInput {
        ...
        if (json.nextToken() != null || targets == null) invalid()
        val requestedStatus = when (action) {
            "STATUS" -> status?.let { ComplaintStatus.valueOf(it) }?.also { if (!ComplaintStateMachine.isStatusTarget(it)) invalid() } ?: invalid()
            "DELETE" -> { if (accepted != Accepted.BOTH || status != null) invalid(); null }
            else -> invalid()
        }
        val operationKey = ComplaintIdentifiers.idempotencyKey(key)
        // Entire structural400 classification precedes ALL missing-tag428 then malformed-tag412.
        ...
        return if (requestedStatus == null) ComplaintAdminBatchInput.Delete(ComplaintAdminBatchDeleteInput(scope, operationKey, targets.mapIndexed { index, raw ->
            ComplaintAdminBatchDeleteTarget(ids[index], ComplaintAdminDeletePrecondition.parse(ids[index], raw.tag))
        })) else ComplaintAdminBatchInput.Status(ComplaintAdminBatchStatusInput(scope, operationKey, requestedStatus, targets.mapIndexed { index, raw ->
...
/** Parsed comparisons only. Each concrete service retains its own admission and operation semantics. */
internal sealed interface ComplaintAdminBatchInput {
    class Status(val input: ComplaintAdminBatchStatusInput) : ComplaintAdminBatchInput
    class Delete(val input: ComplaintAdminBatchDeleteInput) : ComplaintAdminBatchInput
}
