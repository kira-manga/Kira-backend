...
internal object TestRunPurgeRowsV1 {
    ...
    data class Verified(val id: String, val objectRef: TestTerminalObjectRefV1, val createdAt: Instant,
        ...
        override fun toString(): String = "TestRunPurgeRowsV1.Verified(comparison-only,redacted)"
    }
    class Publication(row: ResultSet, original: TestRunPurgePublicationV1) : AutoCloseable {
        ...
        init {
            try {
                requirePurge(TestOrdinaryDrainRowsV1.boolean(row, "valid_shape") && state in setOf("PREPARED", "VERIFIED") &&
                    row.getString("event_kind") == "TEST_RUN_PURGE" && row.getString("canonicalizer") == "kcj-1" &&
                    ...
                    count == 0 && row.getTimestamp("applied_at") == null && Sha256.hex(bytes) == hash)
                ...
                if (state == "VERIFIED") {
                    ...
                    requirePurge(verifiedAt.nano % 1000 == 0 && verifiedAt >= objectCreatedAt && retainUntil > verifiedAt &&
        ...
        fun requireSidecar(sidecar: TestTerminalDurableRowV1, expectedCount: Int) {
            ...
            requirePurge(b.objectId == id && b.objectKey == key && b.routingKeyId == routingKey && b.createdAt == createdAt &&
                count == expectedCount && sidecar.canonicalSha256 == hash)
            val canonical = sidecar.canonicalBytes()
            try { requirePurge(MessageDigest.isEqual(bytes, canonical)) } finally { canonical.fill(0) }
            if (state == "VERIFIED") requirePurge(sidecar.state === TestTerminalDurableStateV1.WIRE_FROZEN && sidecar.wireSha256 == ciphertext &&
                checkNotNull(retainUntil) >= checkNotNull(sidecar.retainUntil))
        }
        fun requireProof(sidecar: TestTerminalDurableRowV1, proof: TestRunPurgeProofV1, expectedCount: Int, now: Instant) {
            requireSidecar(sidecar, expectedCount)
            requirePurge(state == "VERIFIED" && version == proof.version && ciphertext == sidecar.wireSha256 &&
            ...
            try { requirePurge(verification.contentEquals(expected)) } finally { expected.fill(0) }
        }
        fun verifiedFacts(): Verified {
            requirePurge(state == "VERIFIED")
        ...
        fun requireVerifiedReference(expected: Verified, expectedCount: Int, now: Instant) {
            requirePurge(state == "VERIFIED" && count == expectedCount && verifiedFacts() == expected &&
                checkNotNull(retainUntil) > now && checkNotNull(verifiedAt) <= now)
        }
        override fun close() { bytes.fill(0); verification?.fill(0) }
    }

    fun recovery(row: ResultSet, original: TestRunPurgePublicationV1, publication: Publication) {
        requirePurge(TestOrdinaryDrainRowsV1.boolean(row, "valid") && row.getString("event_id") == publication.id &&
            row.getObject("data_scope_id", UUID::class.java) == original.scope && TestOrdinaryDrainRowsV1.boolean(row, "test_only") &&
            row.getTimestamp("created_at").toInstant() == publication.createdAt)
    }

    fun purge(row: ResultSet, original: TestRunPurgePublicationV1, sealedAt: Instant, now: Instant): TestTerminalDurableRowV1 {
        requirePurge(TestOrdinaryDrainRowsV1.boolean(row, "valid"))
        ...
        requirePurge(run == original.runContext && row.getString("object_kind") == "TEST_RUN_PURGE" && row.getInt("object_ordinal") == 0)
        val b = TestTerminalDurableBindingV1(row.getObject("operation_token", UUID::class.java).toString(), run,
            TestOrdinaryDrainRowsV1.hash(row, "journal_configuration_hash"), TestTerminalDurableKindV1.TEST_RUN_PURGE, row.getInt("object_ordinal"),
        ...
        requirePurge(b.writerGeneration == original.writer && b.journalConfigurationSha256 == original.routing.journalConfiguration.sha256 &&
            b.epochStartInclusive == original.epoch && b.epochEndInclusive == original.epoch &&
            b.preparingFencingToken in (original.manifest.leaseToken + 1)..original.leaseToken && b.createdAt >= sealedAt && b.createdAt <= now)
        ...
        try {
            requirePurge(canonical.canonicalSha256 == TestOrdinaryDrainRowsV1.hash(row, "canonical_hash"))
            if (row.getString("state") == "CANONICAL") return canonical
            requirePurge(row.getString("state") == "WIRE_FROZEN")
            ...
            try {
                requirePurge(frozen.frozenAt!! <= now && frozen.retainUntil!! > now && frozen.wireSha256 == TestOrdinaryDrainRowsV1.hash(row, "wire_hash") &&
                ...
                try { requirePurge(expected.contentEquals(actual)) } finally { expected?.fill(0); actual?.fill(0) }
                canonical.close()
                return frozen
