...
/** New closed successor NONPOOLED capture of the already-paid REQUESTED slot. Never C's original producer lineage. */
internal class TestActiveFirstCutSuccessorCaptureOperationV1 private constructor(
    ...
    internal fun belongsTo(selected: TestActiveFirstCutSuccessorV1): Boolean = original === selected
    ...
    internal fun expectedSlot() = original.selectedSlot()
    internal fun readArguments(): Array<Any?> {
        original.requireCapture(this)
        requireFirstCutSuccessor(stage === Stage.LOCKED || stage === Stage.REREADING)
    ...
    internal fun requireReleasedRow(): TestActiveFirstCutStateV1 {
        ...
        original.requireCapture(this)
        return checkNotNull(observed).also { requireFirstCutSuccessor(it.state == "CAPTURED") }
    }
    ...
    private fun execute() {
        try {
            ...
            val accounting = session.lockControl(this) // M/shared -> E/exclusive precedes global/scope/history/P22/run/paid-slot locks.
            requireAt(Stage.RETAINED)
            original.requireAccounting(accounting)
            stage = Stage.LOCKED
            ...
            requireFirstCutSuccessor(before.state == "REQUESTED" && before.sequence == 1L && before.id == original.selectedSlot() &&
                before.epochBefore == 1L && before.epoch == 1L && before.scanRequested && lease.owner == original.owner &&
                lease.token > checkNotNull(before.requestToken) && before.captureOwner == null && before.captureToken == null)
            captureBindings = arrayOf(original.identity.scope, original.selectedSlot(), *lease.arguments(), before.controlFingerprint())
            slotBindings = arrayOf(original.identity.scope, original.selectedSlot(), before.slotFingerprint())
            stage = Stage.WRITING
            ...
            requireFirstCutSuccessor(after.state == "CAPTURED" && after.sequence == 1L && after.epoch == 2L && after.epochAfter == 2L && !after.scanRequested &&
            ...
            val afterAccounting = session.readAccounting(this)
            accounting.requireSame(afterAccounting)
            original.requireAccounting(afterAccounting)
            observed = after
    ...
    private fun requireAt(expected: Stage) {
        original.requireCapture(this)
        requireFirstCutSuccessor(stage === expected)
    }
    override fun toString(): String = "TestActiveFirstCutSuccessorCaptureOperationV1(distinct-successor-exclusive-E,already-paid,no-seal-or-health-authority)"
    private enum class Stage { RETAINED, LOCKED, WRITING, REREADING, COMPLETE, FAILED }

    companion object {
        internal fun execute(original: TestActiveFirstCutSuccessorV1, session: PersistenceEpochRotationSession): TestActiveFirstCutSuccessorCaptureOperationV1 {
            try {
                val operation = TestActiveFirstCutSuccessorCaptureOperationV1(original, session)
