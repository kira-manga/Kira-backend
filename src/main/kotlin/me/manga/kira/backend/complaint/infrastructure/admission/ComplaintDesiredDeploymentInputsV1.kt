...
internal class ComplaintDesiredDeploymentInputsV1 private constructor(document: ComplaintDesiredDeploymentDocumentV1) {
    ...
    internal fun requireTargetSignerRotationAuthorProfile() {
        requireTargetFinalizerProfile()
        valid(profile == DesiredProcessProfileV1.D7 && catalogSignerRotation != null && desiredGeneration == 1L && catalog?.projectedCurrent == false)
    }

    internal fun requireTargetSignerRotationDeliveryProfile() {
        requireTargetFinalizerProfile()
        valid(profile == DesiredProcessProfileV1.D7 && catalogSignerRotation != null && desiredGeneration == 1L && catalog?.projectedCurrent == false)
    }

    internal fun requireTargetSignerRotationActivationProfile() {
        requireTargetFinalizerProfile()
        valid(profile == DesiredProcessProfileV1.D7 && catalogSignerRotation != null && desiredGeneration == 1L && catalog?.projectedCurrent == false)
    }
