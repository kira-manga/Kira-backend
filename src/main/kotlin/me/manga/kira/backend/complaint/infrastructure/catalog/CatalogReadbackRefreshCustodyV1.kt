...
internal class CatalogReadbackRefreshCustodyV1 {
    ...
    internal fun reserveTestActiveInitialCheckpoint(original: TestActiveInitialCheckpointV1) {
        requireConnectionFree(); original.requireCustody(this)
        requireCatalogReadback(active.compareAndSet(null, original), CatalogReadbackFailure.LIMIT_EXCEEDED)
    }
    internal fun requireTestActiveInitialCheckpoint(original: TestActiveInitialCheckpointV1) {
        original.requireCustody(this)
        requireCatalogReadback(active.get() === original, CatalogReadbackFailure.INVALID_POLICY)
    }
    internal fun releaseTestActiveInitialCheckpointAfterCleanup(original: TestActiveInitialCheckpointV1) {
        requireConnectionFree(); original.requireActualReadbackCleanup()
        requireCatalogReadback(active.compareAndSet(original, null), CatalogReadbackFailure.CLOSE_FAILURE)
    }

    internal fun reserveTestActiveRecurrent(original: TestActiveRecurrentV1) {
        requireConnectionFree(); original.requireCustody(this)
        requireCatalogReadback(active.compareAndSet(null, original), CatalogReadbackFailure.LIMIT_EXCEEDED)
    }
    internal fun requireTestActiveRecurrent(original: TestActiveRecurrentV1) {
        original.requireCustody(this)
        requireCatalogReadback(active.get() === original, CatalogReadbackFailure.INVALID_POLICY)
    }
    internal fun releaseTestActiveRecurrentAfterCleanup(original: TestActiveRecurrentV1) {
        requireConnectionFree(); original.requireActualReadbackCleanup()
        requireCatalogReadback(active.compareAndSet(original, null), CatalogReadbackFailure.CLOSE_FAILURE)
    }

    internal fun reserveTestActiveOwnerDeleteQueue(original: TestActiveOwnerDeleteQueueV1) {
        requireConnectionFree(); original.requireCustody(this)
        requireCatalogReadback(active.compareAndSet(null, original), CatalogReadbackFailure.LIMIT_EXCEEDED)
    }
    internal fun requireTestActiveOwnerDeleteQueue(original: TestActiveOwnerDeleteQueueV1) {
        original.requireCustody(this)
        requireCatalogReadback(active.get() === original, CatalogReadbackFailure.INVALID_POLICY)
    }
    internal fun releaseTestActiveOwnerDeleteQueueAfterCleanup(original: TestActiveOwnerDeleteQueueV1) {
        requireConnectionFree(); original.requireActualReadbackCleanup()
        requireCatalogReadback(active.compareAndSet(original, null), CatalogReadbackFailure.CLOSE_FAILURE)
    }

    internal fun reserveTestActiveOrdinarySealRecovery(original: TestActiveOrdinarySealRecoveryV1) {
        requireConnectionFree(); original.requireCustody(this)
        requireCatalogReadback(active.compareAndSet(null, original), CatalogReadbackFailure.LIMIT_EXCEEDED)
    }
    internal fun requireTestActiveOrdinarySealRecovery(original: TestActiveOrdinarySealRecoveryV1) {
        original.requireCustody(this)
        requireCatalogReadback(active.get() === original, CatalogReadbackFailure.INVALID_POLICY)
    }
    internal fun releaseTestActiveOrdinarySealRecoveryAfterCleanup(original: TestActiveOrdinarySealRecoveryV1) {
        requireConnectionFree(); original.requireActualReadbackCleanup()
        requireCatalogReadback(active.compareAndSet(original, null), CatalogReadbackFailure.CLOSE_FAILURE)
    }
    ...
    internal fun reserveTestActiveFirstCutSuccessor(original: TestActiveFirstCutSuccessorV1) {
        requireConnectionFree()
        original.requireCustody(this)
        requireCatalogReadback(active.compareAndSet(null, original), CatalogReadbackFailure.LIMIT_EXCEEDED)
    }

    internal fun requireTestActiveFirstCutSuccessor(original: TestActiveFirstCutSuccessorV1) {
        original.requireCustody(this)
        requireCatalogReadback(active.get() === original, CatalogReadbackFailure.INVALID_POLICY)
    }

    internal fun releaseTestActiveFirstCutSuccessorAfterCleanup(original: TestActiveFirstCutSuccessorV1) {
        requireConnectionFree()
        original.requireCustody(this)
        original.requireActualCleanup()
        requireCatalogReadback(active.compareAndSet(original, null), CatalogReadbackFailure.CLOSE_FAILURE)
    ...
    internal fun reserveTestRecoveryRegistration(original: ComplaintTestNamespaceRecoveryRegistrationAttemptV1) {
        requireConnectionFree()
        original.requireCustody(this)
        requireCatalogReadback(active.compareAndSet(null, original), CatalogReadbackFailure.LIMIT_EXCEEDED)
    }

    internal fun requireTestRecoveryRegistration(original: ComplaintTestNamespaceRecoveryRegistrationAttemptV1) {
        original.requireCustody(this)
        requireCatalogReadback(active.get() === original, CatalogReadbackFailure.INVALID_POLICY)
    }

    internal fun releaseTestRecoveryRegistrationAfterCleanup(original: ComplaintTestNamespaceRecoveryRegistrationAttemptV1) {
        requireConnectionFree()
        original.requireCustody(this)
        original.requireActualCleanup()
        requireCatalogReadback(active.compareAndSet(original, null), CatalogReadbackFailure.CLOSE_FAILURE)
    ...
    internal fun reserveTestActiveRegistration(original: ComplaintTestNamespaceActiveRegistrationAttemptV1) {
        requireConnectionFree()
        original.requireCustody(this)
        requireCatalogReadback(active.compareAndSet(null, original), CatalogReadbackFailure.LIMIT_EXCEEDED)
    }

    internal fun requireTestActiveRegistration(original: ComplaintTestNamespaceActiveRegistrationAttemptV1) {
        original.requireCustody(this)
        requireCatalogReadback(active.get() === original, CatalogReadbackFailure.INVALID_POLICY)
    }

    internal fun releaseTestActiveRegistrationAfterCleanup(original: ComplaintTestNamespaceActiveRegistrationAttemptV1) {
        requireConnectionFree()
        original.requireCustody(this)
        original.requireActualCleanup()
        requireCatalogReadback(active.compareAndSet(original, null), CatalogReadbackFailure.CLOSE_FAILURE)
