...
/**
 * Distinct already-open ACTIVE identity admission from the actual protected cold assembly. Never
 * replays PROJECT, consumes a first-projection ticket, reopens gates or changes writer generation.
 ...
 */
internal class ComplaintTestNamespaceActiveRegistrationAttemptV1 private constructor(
    ...
    private var captured: TestNamespaceActiveRegistrationOperationV1? = null
    private var capturedSnapshot: TestNamespaceActiveRegistrationSnapshotV1? = null
    private var rechecked: TestNamespaceActiveRegistrationOperationV1? = null
    ...
    init {
        ...
        process.claimActiveRegistration(this)
    }
    ...
    fun register(primary: AwsSessionCredentials, replica: AwsSessionCredentials): ComplaintTestNamespaceRegistrationV1 {
        ...
        try {
            ...
            coordinator.catalogRefreshCustody.reserveTestActiveRegistration(this)
            reserved = true
            stage = Stage.CAPTURE
            captured = coordinator.testNamespaceActiveRegistration.execute(this)
            capturedSnapshot = checkNotNull(captured).snapshot
            stage = Stage.READBACK
            observe(primary, replica)
            stage = Stage.RECHECK
            rechecked = coordinator.testNamespaceActiveRegistration.execute(this)
        ...
        return ComplaintTestNamespaceRegistrationV1.issuedByActiveRegistration(this)
    }
    ...
    private fun observe(primary: AwsSessionCredentials, replica: AwsSessionCredentials) {
        ...
        val read = withSignerRotationCleanup({
            ...
            CatalogTestRunActivationReadbackV3.verifyActiveRegistration(TimedReadback(adapter), reader.initialBundleBytes(), reader.currentBundleBytes(), policy,
    ...
    internal fun requirePhaseEntry(ownership: PersistencePhaseOwnership, path: PersistencePhasePath) {
        requireConnectionFree()
        requireRunning()
        requireRegistration(ownership === coordinator.ownership && path === PersistencePhasePath.COMPLAINT_TEST_NAMESPACE_ACTIVE_REGISTRATION &&
    ...
    internal fun observePhaseCleanup(selected: PersistencePhaseContext) {
        try {
            if (caller !== Thread.currentThread() || phase !== selected || !selected.testActiveRegistrationCleanupProven(this) ||
    ...
    internal fun requireMaintenanceGate(ownership: PersistencePhaseOwnership, path: PersistencePhasePath, gate: PersistenceComplaintMaintenanceGateV1) {
        requireRunning()
        requireRegistration(ownership === coordinator.ownership && path === PersistencePhasePath.COMPLAINT_TEST_NAMESPACE_ACTIVE_REGISTRATION && phaseEntered && phase != null)
        requireRegistration(this.gate == null && gate.matchesOpenProjectedActiveRegistrationScope(process.consumers.journalConfiguration.scope.id))
        capturedSnapshot?.tail?.let { requireActiveGate(it, gate) }
        this.gate = gate
    }
    ...
    internal fun requireCapturedGate(tail: TestNamespaceRecoveryRegistrationTailV1) {
        requireRunning()
        requireActiveGate(tail, checkNotNull(gate))
    }

    private fun requireActiveGate(tail: TestNamespaceRecoveryRegistrationTailV1, gate: PersistenceComplaintMaintenanceGateV1) =
        requireRegistration(gate.matchesOpenProjectedActive(tail.token, tail.scope, tail.unsigned, tail.unsignedHash))
    ...
    private fun requireRunning() {
        ...
        process.requireRegistrationTarget()
        if (reserved) coordinator.catalogRefreshCustody.requireTestActiveRegistration(this)
    }

    internal fun observeFailure(problem: Throwable) {
        val retained = when {
            problem is Error -> problem
            problem is CancellationException -> CancellationException("TEST ACTIVE registration cancelled.")
            problem is InterruptedException || problem is InterruptedIOException ||
                ...
                InterruptedException("TEST ACTIVE registration interrupted.")
            else -> return
        }
        while (true) {
    ...
    override fun close() {
        ...
        cleanupProven = true
        if (reserved) coordinator.catalogRefreshCustody.releaseTestActiveRegistrationAfterCleanup(this)
        released = true
        closeFailure = null
    }
    ...
    internal fun consumeRegistration(): TestNamespaceActiveRegistrationBindingV1 {
        requireActualCleanup()
        requireRegistration(successful && released && closeFailure == null && !issued && proof != null && providerStarted && providerClosed && providerFailure == null)
    ...
    override fun toString(): String = "ComplaintTestNamespaceActiveRegistrationAttemptV1(original-cold-runtime,identity-only,redacted)"
    private enum class Stage { NEW, CAPTURE, READBACK, RECHECK }

    companion object {
        fun begin(assembly: ComplaintTestProcessAssemblyV1): ComplaintTestNamespaceActiveRegistrationAttemptV1 =
            ComplaintTestNamespaceActiveRegistrationAttemptV1(assembly, null, Clock.systemUTC())
        ...
        internal fun withHttpFixture(assembly: ComplaintTestProcessAssemblyV1, http: () -> SdkHttpClient,
            clock: Clock): ComplaintTestNamespaceActiveRegistrationAttemptV1 =
            ComplaintTestNamespaceActiveRegistrationAttemptV1(assembly, http, clock)
    }
}
