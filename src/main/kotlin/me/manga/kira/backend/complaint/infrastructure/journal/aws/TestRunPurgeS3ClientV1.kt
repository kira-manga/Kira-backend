...
/** Closed TEST purge facade; frozen original-holder release is checked on every wire call; raw HTTP bounds, XML inspection, observations and native cleanup are shared. */
internal class TestRunPurgeS3ClientV1 private constructor(
    ...
    fun listExact(binding: TestRunPurgeS3BindingV1): JournalListedVersionV1? {
        val call = TestRunPurgeS3CallV1.list(binding, nanoTime)
    ...
    fun putIfAbsent(binding: TestRunPurgeS3BindingV1, candidate: TestRunPurgeS3CandidateV1): JournalPutObservationV1 {
        val call = TestRunPurgeS3CallV1.put(binding, candidate, nanoTime)
        val location = call.declaration.journalLocation
        return execute(call) {
    ...
    fun getVersion(binding: TestRunPurgeS3BindingV1, versionId: String): JournalFetchedVersionV1 {
        val call = TestRunPurgeS3CallV1.get(binding, versionId, nanoTime)
        val location = call.declaration.journalLocation
    ...
    private fun <T> execute(call: TestRunPurgeS3CallV1, action: () -> T): T = journalPublicationSdkCall {
    ...
    internal class Construction : AutoCloseable {
        ...
        private var owner: TestRunPurgeS3ClientV1? = null
        ...
        fun open(routing: TestOwnerDeleteJournalRoutingV1, credentials: AwsSessionCredentials, httpFactory: (remainingMillis: () -> Int) -> SdkHttpClient,
            nanoTime: () -> Long, attempt: TestTerminalAttemptV1): TestRunPurgeS3ClientV1 = journalPublicationSdkCall {
            requireConnectionFree()
            ...
            val result = runCatching {
                ...
                attempt.remainingMillis(1)
                TestRunPurgeS3ClientV1(routing, client, bounded, nanoTime).also { owner = it }
            }
    ...
    private companion object {
        const val PROFILE = "complaint-test-run-purge-s3-v1"
        fun overrides(call: TestRunPurgeS3CallV1): AwsRequestOverrideConfiguration {
