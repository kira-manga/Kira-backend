 ...
 */
internal class LinuxGenesisReleaseFilesV1(private val owner: CatalogGenesisReleaseCustodyV1, private val selectedRoot: Path) {
    ...
    fun openRoot(): Any {
        requireGenesisCustody(
            ...
            CatalogGenesisCustodyFailureV1.UNSUPPORTED_FILESYSTEM,
        )
    ...
    fun openAllocation(expected: ByteArray): CatalogGenesisCustodyObservationV1 {
        ...
        checkDirectories()
        if (created) {
            requireGenesisCustody(scan(allocationDirectory(), LEAF_NAMES).isEmpty(), CatalogGenesisCustodyFailureV1.INVENTORY_REFUSED)
        ...
        return if (created) CatalogGenesisCustodyObservationV1.CREATED else CatalogGenesisCustodyObservationV1.IDENTICAL_OBSERVED
    }

    fun putIfAbsent(expectedAllocation: ByteArray, leaf: CatalogGenesisReleaseLeafV1, bytes: ByteArray): CatalogGenesisCustodyObservationV1 {
        val existing = readInventory(expectedAllocation)[leaf.fileName]
        if (existing != null) {
            requireGenesisCustody(existing.contentEquals(bytes), CatalogGenesisCustodyFailureV1.DIFFERENT_BYTES)
            return CatalogGenesisCustodyObservationV1.IDENTICAL_OBSERVED
        }
        writePair(Spec(leaf.fileName, leaf.maximumBytes), bytes)
        val reread = readInventory(expectedAllocation)[leaf.fileName]
        requireGenesisCustody(reread != null && reread.contentEquals(bytes), CatalogGenesisCustodyFailureV1.DIFFERENT_BYTES)
        return CatalogGenesisCustodyObservationV1.CREATED
    }

    fun read(expectedAllocation: ByteArray, leaf: CatalogGenesisReleaseLeafV1): ByteArray? = readInventory(expectedAllocation)[leaf.fileName]

    /**
     * Cleanup ignores deadlines but never retries an uncertain close, opens a replacement or deletes.
     * Catch even fatal Errors to finish closing the other original handles; the owner then rethrows the first fatal.
     */
    ...
    fun closeAll(): Error? {
        ...
        for (index in resources.lastIndex downTo 0) {
            ...
            try {
                ...
                CatalogGenesisReleaseCustodyV1.preserveInterruption(failure)
                if (failure is Error && fatal == null) fatal = failure
            }
        }
        return fatal
    }
    ...
    private fun openDirectory(path: Path, parent: Directory?, privateBoundary: Boolean): Directory {
        ...
        if (before != null) requireGenesisCustody(before.isDirectory && !before.isSymbolicLink, CatalogGenesisCustodyFailureV1.UNPROTECTED_PATH)
        ...
        val secure = held.value as? SecureDirectoryStream<Path>
            ?: throw CatalogGenesisCustodyExceptionV1(CatalogGenesisCustodyFailureV1.UNSUPPORTED_FILESYSTEM)
        val attributes = selfAttributes(secure)
        requireGenesisCustody(
            attributes.isDirectory && (before == null || key(before) == key(attributes)),
            CatalogGenesisCustodyFailureV1.UNPROTECTED_PATH,
        )
    ...
    private fun checkDirectory(directory: Directory) {
        ...
        val mode = unix["mode"] as? Int ?: throw CatalogGenesisCustodyExceptionV1(CatalogGenesisCustodyFailureV1.UNSUPPORTED_FILESYSTEM)
    ...
    private fun scan(directory: Directory, allowed: Set<String>): Set<String> {
        ...
        requireGenesisCustody(key(selfAttributes(stream)) == directory.key, CatalogGenesisCustodyFailureV1.UNPROTECTED_PATH)
        val names = mutableSetOf<String>()
        val iterator = io { stream.iterator() }
        while (io { iterator.hasNext() }) {
            val entry = io { iterator.next() }
            val name = entry.fileName.toString()
            requireGenesisCustody(names.size < allowed.size && name in allowed && names.add(name), CatalogGenesisCustodyFailureV1.INVENTORY_REFUSED)
        }
        closeTransient(held)
        checkDirectory(directory)
        return names
    ...
    private fun openPermanentLock(exists: Boolean) {
        ...
        requireGenesisCustody(resources.size < MAX_RESOURCES, CatalogGenesisCustodyFailureV1.INVALID_STATE)
        ...
        if (lockHeld.value == null) {
            ...
            throw CatalogGenesisCustodyExceptionV1(CatalogGenesisCustodyFailureV1.LOCK_UNAVAILABLE)
        }
        lock = lockHeld.value
        owner.checkpoint(this)
        requireLiveLock()
    ...
    private fun requireLiveLock() {
        ...
        requireGenesisCustody(
            ...
            CatalogGenesisCustodyFailureV1.LOCK_UNAVAILABLE,
        )
    ...
    private fun readInventory(expectedAllocation: ByteArray): Map<String, ByteArray> {
        requireLiveLock()
        checkDirectories()
        requireGenesisCustody(scan(rootDirectory(), ROOT_NAMES) == ROOT_NAMES, CatalogGenesisCustodyFailureV1.INCOMPLETE)
        ...
        for (spec in SPECS) {
            ...
            requireGenesisCustody(present == (spec.completenessName in names), CatalogGenesisCustodyFailureV1.INCOMPLETE)
            if (present) result[spec.name] = readPair(spec)
        }
        requireGenesisCustody(result[ALLOCATION.name]?.contentEquals(expectedAllocation) == true, CatalogGenesisCustodyFailureV1.DIFFERENT_BYTES)
        checkDirectories()
        requireLiveLock()
        return result
    }
    ...
    private fun readPair(spec: Spec): ByteArray {
        ...
        requireGenesisCustody(sidecar.contentEquals(completeness(bytes)), CatalogGenesisCustodyFailureV1.INCOMPLETE)
        return bytes
    }
    ...
    private fun readSealed(name: String, maximumBytes: Int): ByteArray {
        ...
        requireGenesisCustody(before.size > 0, CatalogGenesisCustodyFailureV1.INCOMPLETE)
        val held = capture { directory.stream.newByteChannel(Path.of(name), READ_OPTIONS) }
        val channel = fileChannel(held)
        requireGenesisCustody(io { channel.size() } == before.size, CatalogGenesisCustodyFailureV1.INCOMPLETE)
        val output = ByteBuffer.allocate(before.size.toInt())
        while (output.hasRemaining()) requireGenesisCustody(io { channel.read(output) } > 0, CatalogGenesisCustodyFailureV1.INCOMPLETE)
        requireGenesisCustody(io { channel.read(ByteBuffer.allocate(1)) } == -1, CatalogGenesisCustodyFailureV1.INCOMPLETE)
        requireGenesisCustody(io { channel.size() } == before.size, CatalogGenesisCustodyFailureV1.INCOMPLETE)
        closeTransient(held)
        val after = checkedFile(directory, name, maximumBytes, READ_MODE)
        requireGenesisCustody(before == after, CatalogGenesisCustodyFailureV1.UNPROTECTED_PATH)
        return output.array()
    }
    ...
    private fun writeSealed(name: String, bytes: ByteArray, maximumBytes: Int) {
        ...
        requireGenesisCustody(created.size == 0L && io { channel.size() } == 0L, CatalogGenesisCustodyFailureV1.INCOMPLETE)
        val input = ByteBuffer.wrap(bytes)
        while (input.hasRemaining()) requireGenesisCustody(io { channel.write(input) } > 0, CatalogGenesisCustodyFailureV1.IO_UNCERTAIN)
        val written = checkedFile(directory, name, maximumBytes, WRITE_MODE)
        requireGenesisCustody(written.key == created.key && written.size == bytes.size.toLong(), CatalogGenesisCustodyFailureV1.INCOMPLETE)
        io {
        ...
        requireGenesisCustody(sealed == written, CatalogGenesisCustodyFailureV1.UNPROTECTED_PATH)
        io { channel.force(true) }
        closeTransient(held)
        forceDirectory(directory)
        requireGenesisCustody(readSealed(name, maximumBytes).contentEquals(bytes), CatalogGenesisCustodyFailureV1.DIFFERENT_BYTES)
    }
    ...
    private fun checkedFile(directory: Directory, name: String, maximumBytes: Int, expectedMode: Int): FileIdentity {
        ...
        requireGenesisCustody(fileKeys[name] == null || fileKeys[name] == identity.key, CatalogGenesisCustodyFailureV1.UNPROTECTED_PATH)
    ...
    private fun closeRetained(held: Held<*>) {
        requireGenesisCustody(!held.closeIssued, CatalogGenesisCustodyFailureV1.CLEANUP_UNCERTAIN)
        held.closeIssued = true
        val value = held.value ?: throw CatalogGenesisCustodyExceptionV1(CatalogGenesisCustodyFailureV1.CLEANUP_UNCERTAIN)
        value.close()
    ...
    companion object {
        ...
        private const val LOCK_FILE = ".custody.lock"
        private const val ALLOCATION_DIRECTORY = "genesis"
        ...
        private val ALLOCATION = Spec("allocation", CatalogGenesisCapacity.MAX_DOCUMENT_BYTES)
        private val SPECS = listOf(ALLOCATION) + CatalogGenesisReleaseLeafV1.entries.map { Spec(it.fileName, it.maximumBytes) }
