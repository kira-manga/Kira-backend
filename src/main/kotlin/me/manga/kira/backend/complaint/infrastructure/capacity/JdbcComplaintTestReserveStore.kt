 ...
 */
internal class JdbcComplaintTestReserveStore(private val jdbc: JdbcTemplate, private val capacity: JdbcComplaintCapacityStore) : ComplaintTestReserveSpend {
    override fun spend(expectation: ComplaintTestReserveSpendExpectation): ComplaintTestReserveSpendResult =
        ComplaintTestReserveSpendOperation.prepare(jdbc, expectation).spend(capacity)
...
internal class ComplaintTestReserveSpendOperation private constructor(
    ...
    internal fun spend(capacity: JdbcComplaintCapacityStore): ComplaintTestReserveSpendResult {
        try {
            ...
            run = jdbc.query(LOCK_RUN, { row, _ -> readRun(row, expectation) }, expectation.scope.id).single()
    ...
    internal fun spendLockedLedger(selected: JdbcTemplate, ledger: ComplaintCapacityLedger, expectedDigest: ByteArray): ComplaintCapacityLedger {
        ...
        original.configuration.requireMatching(expectedDigest)
    ...
    private fun updateUnused() {
        ...
        val updated = jdbc.update(
            ...
            original.configuration.digestBytes(),
    ...
    private class LockedRun(val configuration: ComplaintCapacityConfiguration, val unused: ComplaintCapacityVector)
    ...
    companion object {
        ...
        @Suppress("TooGenericExceptionCaught")
        internal fun prepare(jdbc: JdbcTemplate, expectation: ComplaintTestReserveSpendExpectation): ComplaintTestReserveSpendOperation {
            val phase = PersistencePhaseOwnership.current() ?: throw PersistencePhaseException(PersistencePhaseFailureCode.ENTRY_REFUSED)
            try {
                phase.requireComplaintTestReserveSpend(jdbc)
                val operation = ComplaintTestReserveSpendOperation(phase, jdbc, expectation)
        ...
        private fun readRun(row: ResultSet, expected: ComplaintTestReserveSpendExpectation): LockedRun {
            ...
            return LockedRun(ComplaintCapacityConfiguration.of(row.getBytes("configuration_hash"), true), unused)
        }
