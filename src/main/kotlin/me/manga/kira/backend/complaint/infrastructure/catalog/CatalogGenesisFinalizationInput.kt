...
/** Only the opaque raw verifier handoff can select these fixed SQL parameters; never diagnostic readback results. */
internal class CatalogGenesisFinalizationInput private constructor(readback: CatalogDualLocationVerifier.GenesisReadback) {
    private val manifest = readback.manifest()
    ...
    private val configurationHash = HexFormat.of().parseHex(manifest.initialWriterRegistry.eventWriter.liveRange.configurationSha256)
    ...
    internal fun controlArguments(): Array<Any?> {
        requireConnectionFree()
        return arrayOf(envelopeHash.copyOf(), trustHash.copyOf(), writer, token, databaseIdentity, restoreIdentity, eventWriter, configurationHash.copyOf())
    }
    ...
    companion object {
        internal fun verified(readback: CatalogDualLocationVerifier.GenesisReadback): CatalogGenesisFinalizationInput {
            requireConnectionFree()
            return CatalogGenesisFinalizationInput(readback)
        }
