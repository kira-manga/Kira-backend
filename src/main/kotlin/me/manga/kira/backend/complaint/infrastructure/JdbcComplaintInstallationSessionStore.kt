...
/** Fixed LIVE-only database core. No bean, JWT, current-mode proof or connection-free semantic admission is supplied here. */
internal class JdbcComplaintInstallationSessionStore(private val jdbc: JdbcTemplate) : ComplaintInstallationSession {
    private val issuer = Any()

    override fun preflight(candidate: InstallationSessionCandidate): SessionPreflightResult =
        ComplaintInstallationSessionOperation.preparePreflight(jdbc, issuer, candidate).preflight()

    override fun refresh(preflight: InstallationSessionPreflight): SessionRefreshResult =
        ComplaintInstallationSessionOperation.prepareRefresh(jdbc, issuer, preflight).refresh()
...
internal class ComplaintInstallationSessionOperation private constructor(
    ...
    internal fun preflight(): SessionPreflightResult {
        try {
            ...
            val rejection = pair.rejection(candidate, null)
            val completed = if (rejection != null) {
                SessionPreflightResult.Rejected(rejection)
            } else {
                SessionPreflightResult.Ready(
                    Continuation(candidate, checkNotNull(pair.credential).credentialVersion, issuer, phase.installationSession.ownerIdentity(this, jdbc)),
                )
            }
            result = completed
    ...
    internal fun refresh(): SessionRefreshResult {
        try {
            ...
            val rejection = StoredPair(identity, credential).rejection(candidate, expectedCredentialVersion)
    ...
    private enum class Stage { PREPARED, READING_SNAPSHOT, LOCKING_IDENTITY, LOCKING_CREDENTIAL, SAMPLING_TIME, REFRESHING, COMPLETE, FAILED }
    ...
    private class StoredPair(val identity: StoredIdentity?, val credential: StoredCredential?) {
        fun rejection(candidate: InstallationSessionCandidate, expectedVersion: Long?): InstallationSessionRejection? {
            requireCoherentPair()
            // Unsupported comparison scopes are normal rejections, never authority to fall back to LIVE.
            if (candidate.installation.scope.testOnly) return InstallationSessionRejection.INSTALLATION_SCOPE_MISMATCH
    ...
    companion object {
        @Suppress("TooGenericExceptionCaught")
        internal fun preparePreflight(jdbc: JdbcTemplate, issuer: Any, candidate: InstallationSessionCandidate): ComplaintInstallationSessionOperation {
        ...
        @Suppress("TooGenericExceptionCaught")
        internal fun prepareRefresh(jdbc: JdbcTemplate, issuer: Any, preflight: InstallationSessionPreflight): ComplaintInstallationSessionOperation {
            val phase = PersistencePhaseOwnership.current() ?: throw PersistencePhaseException(PersistencePhaseFailureCode.ENTRY_REFUSED)
            try {
