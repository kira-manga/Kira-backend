...
import me.manga.kira.backend.complaint.infrastructure.journal.aws.TestInstallationManifestS3BindingV1
import me.manga.kira.backend.complaint.infrastructure.journal.aws.TestInstallationManifestS3CandidateV1
...
internal class TestInstallationManifestProofV1 private constructor(
    private val custody: TestInstallationManifestCustodyV1,
    val version: String,
    ...
    internal fun canonicalBytes(row: TestTerminalDurableRowV1, at: Instant = verifiedAt): ByteArray {
        ...
        return CanonicalJson.canonicalize(buildJsonObject {
            put("schemaVersion", 1); put("objectKind", "INSTALLATION_MANIFEST"); put("objectOrdinal", row.binding.objectOrdinal)
    ...
    override fun toString(): String = "TestInstallationManifestProofV1(authenticated-exact-version,manifest-chunk-only,redacted)"

    companion object {
        internal fun publish(custody: TestInstallationManifestCustodyV1, s3: TestInstallationManifestS3ClientV1, keys: AwsTestTerminalDataKeyAdapterV1): TestInstallationManifestProofV1 {
            custody.requirePublication()
            val binding = TestInstallationManifestS3BindingV1.released(custody)
            ...
            return withJournalPublicationCleanup({
                ...
                requireManifest(decoded.content === custody.content() && decoded.wireSha256 == frozen.wireSha256)
                ...
                custody.requirePublication()
                TestInstallationManifestProofV1(custody, exact.versionId, facts.first, facts.second, now)
            }, fetched::close)
        }
        ...
        private fun cheapChecks(row: TestTerminalDurableRowV1, listed: JournalListedVersionV1, fetched: JournalFetchedVersionV1): Pair<Instant, Instant> {
            ...
            requireManifest(raw.statusCode() == 200 && response.sdkHttpResponse().statusCode() == 200 &&
            ...
            try { requireManifest(MessageDigest.isEqual(wire, fetched.bytes)) } finally { wire.fill(0) }
            requireManifest(response.contentLength() == listed.size && fetched.bytes.size.toLong() == listed.size &&
            ...
            requireManifest(response.deleteMarker() != true && response.contentRange() == null && response.contentEncoding() == null && response.expiration() == null)
            requireManifest(response.contentType() == JournalS3HttpWireV1.CONTENT_TYPE && JournalS3HttpWireV1.single(headers, "Content-Type") == JournalS3HttpWireV1.CONTENT_TYPE)
            requireManifest(response.metadata() == row.metadata() && JournalS3HttpWireV1.metadata(headers) == row.metadata() && (response.missingMeta() == null || response.missingMeta() == 0))
            val checksum = JournalS3HttpWireV1.single(headers, "x-amz-checksum-sha256")
            requireManifest(checksum == row.checksumSha256 && response.checksumSHA256() == checksum &&
            ...
            try { requireManifest(Base64.getEncoder().encodeToString(hash) == checksum) } finally { hash.fill(0) }
            requireManifest(response.objectLockModeAsString() == "COMPLIANCE" && JournalS3HttpWireV1.single(headers, "x-amz-object-lock-mode") == "COMPLIANCE")
            val created = checkNotNull(response.lastModified())
            val retained = checkNotNull(response.objectLockRetainUntilDate())
            requireManifest(created == listed.lastModified &&
