...
internal object TestOrdinaryDrainAdminPersistenceV1 {
    fun requirePrimaryFacts(jdbc: JdbcTemplate, facts: TestOrdinaryDrainPersistenceV1.FamilyFacts, run: TestOrdinaryDrainRowsV1.Run,
        ...
        requireDrain(facts.routing.journalConfiguration.registeredAdminDelete && p.kind.name == "ADMIN_DELETE")
        ...
        val proof = TestOwnerDeleteVerificationCodecV1.forAdmin(facts.routing).parse(checkNotNull(p.verificationBytes), event)
        ...
        if (ownerState == "DELETED") {
            ...
            requireDrain(TestOrdinaryDrainPersistenceV1.requireCompletedAllCompanionFacts(jdbc, facts, run, comparison.ownerInstallationId))
        } else {
