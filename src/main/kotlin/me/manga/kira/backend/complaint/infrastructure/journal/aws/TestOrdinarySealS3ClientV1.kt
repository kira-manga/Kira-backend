...
/** Closed TEST seal facade; frozen original-holder release is checked on every wire call; raw HTTP bounds, XML inspection, observations and native cleanup are shared. */
internal class TestOrdinarySealS3ClientV1 private constructor(
    ...
    fun listExact(binding: TestOrdinarySealS3BindingV1): JournalListedVersionV1? {
        val call = TestOrdinarySealS3CallV1.list(binding, nanoTime)
    ...
    fun putIfAbsent(binding: TestOrdinarySealS3BindingV1, candidate: TestOrdinarySealS3CandidateV1): JournalPutObservationV1 {
        val call = TestOrdinarySealS3CallV1.put(binding, candidate, nanoTime)
        val location = call.declaration.journalLocation
        return execute(call) {
    ...
    fun getVersion(binding: TestOrdinarySealS3BindingV1, versionId: String): JournalFetchedVersionV1 {
        val call = TestOrdinarySealS3CallV1.get(binding, versionId, nanoTime)
        val location = call.declaration.journalLocation
    ...
    private fun <T> execute(call: TestOrdinarySealS3CallV1, action: () -> T): T = journalPublicationSdkCall {
    ...
    internal class Construction : AutoCloseable {
        ...
        private var owner: TestOrdinarySealS3ClientV1? = null
        ...
        fun open(routing: TestOwnerDeleteJournalRoutingV1, credentials: AwsSessionCredentials, httpFactory: (remainingMillis: () -> Int) -> SdkHttpClient,
            nanoTime: () -> Long, attempt: TestTerminalAttemptV1): TestOrdinarySealS3ClientV1 = journalPublicationSdkCall {
            requireConnectionFree()
            ...
            val result = runCatching {
                ...
                attempt.remainingMillis(1)
                TestOrdinarySealS3ClientV1(routing, client, bounded, nanoTime).also { owner = it }
            }
    ...
    private companion object {
        const val PROFILE = "complaint-test-ordinary-seal-s3-v1"
        fun overrides(call: TestOrdinarySealS3CallV1): AwsRequestOverrideConfiguration {
