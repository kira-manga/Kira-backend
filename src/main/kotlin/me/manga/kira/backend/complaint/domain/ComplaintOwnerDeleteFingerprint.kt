...
internal class ComplaintOwnerDeleteFingerprint private constructor(private val digest: ByteArray) {
    ...
    companion object {
        ...
        const val ROUTE = "/api/v1/complaints/{id}"

        fun of(request: ComplaintOwnerDeleteRequest): ComplaintOwnerDeleteFingerprint {
            val frame = frameBytes(request)
            return try {
                ComplaintOwnerDeleteFingerprint(MessageDigest.getInstance("SHA-256").digest(frame))
            } finally {
                frame.fill(0)
            }
        }
        ...
        internal fun frameBytes(request: ComplaintOwnerDeleteRequest): ByteArray {
            ...
            DataOutputStream(buffer).use { frame ->
                ...
                frame.field("DELETE")
                frame.field(ROUTE)
                frame.field(ComplaintOwnerDeleteTuple.OPERATION)
