...
/**
 * Real pinned SDK, all-version maxKeys2 first-seal/whole-prefix LIST and exact seal GET only.
 * The whole ordinary prefix is checked, never a filtered writer/key subset or continuation. Raw bodies/XML are bounded before SDK
 * decoding; every exchange finishes before a page/body reaches the reader. There is no PUT API.
 */
internal class TestActiveInitialCheckpointS3ClientV1 private constructor(
    private val reader: TestActiveInitialCheckpointReaderV1,
    ...
    fun listExactSeal(): JournalListedVersionV1 {
        val call = TestActiveInitialCheckpointS3CallV1.listSeal(reader, nanoTime)
        val response = list(call)
        return journalPublicationCall(JournalPublicationFailureV1.INVALID_LISTING) {
            call.check()
            val version = response.versions().single()
            requireJournalPublication(version.key() == call.objectKey && version.isLatest() == true &&
                version.versionId() == reader.sealVersion() && version.size() != null &&
                version.size() in 1..call.declaration.limits.decoder.maximumEnvelopeBytes.toLong() && version.lastModified() != null)
            call.check()
            JournalListedVersionV1(requireJournalVersion(version.versionId()), version.size(), version.lastModified())
        }
    }

    /** One genuine whole-prefix all-version request; any object/delete marker/truncation refuses, never filters. */
    fun requireEmptyOrdinaryPrefix() {
        val call = TestActiveInitialCheckpointS3CallV1.listOrdinary(reader, nanoTime)
        val response = list(call)
        requireJournalPublication(response.versions().isEmpty(), JournalPublicationFailureV1.INVALID_LISTING)
        call.check()
    }

    private fun list(call: TestActiveInitialCheckpointS3CallV1): software.amazon.awssdk.services.s3.model.ListObjectVersionsResponse {
        val location = call.declaration.journalLocation
        val response = execute(call) {
            sdk.listObjectVersions(ListObjectVersionsRequest.builder().bucket(location.bucket).expectedBucketOwner(location.accountId)
                .prefix(call.objectKey).maxKeys(2).encodingType(EncodingType.URL).overrideConfiguration(overrides(call)).build()).also {
                    requireJournalPublication(transport.observation(call)?.response?.statusCode() == 200, JournalPublicationFailureV1.INVALID_LISTING)
                }
        }
        call.check()
        requireJournalPublication(response.sdkHttpResponse().statusCode() == 200 && response.name() == location.bucket &&
            response.prefix() == call.objectKey && response.maxKeys() == 2 && response.encodingTypeAsString() == "url" && response.isTruncated() == false,
            JournalPublicationFailureV1.INVALID_LISTING)
        requireJournalPublication(response.keyMarker().isNullOrEmpty() && response.versionIdMarker().isNullOrEmpty() &&
    ...
    fun getExactSeal(): JournalFetchedVersionV1 {
        val call = TestActiveInitialCheckpointS3CallV1.getSeal(reader, nanoTime)
        val location = call.declaration.journalLocation
    ...
    private fun <T> execute(call: TestActiveInitialCheckpointS3CallV1, action: () -> T): T = journalPublicationSdkCall {
    ...
    override fun toString(): String = "TestActiveInitialCheckpointS3ClientV1(original-read-only,redacted)"
    ...
    internal class Construction : AutoCloseable {
        ...
        private var owner: TestActiveInitialCheckpointS3ClientV1? = null
        private var rawCloseIssued = false
        private var sdkCloseIssued = false
        private var failure: Throwable? = null

        fun open(
            reader: TestActiveInitialCheckpointReaderV1,
            credentials: AwsSessionCredentials,
            httpFactory: (remainingMillis: () -> Int) -> SdkHttpClient,
            nanoTime: () -> Long,
        ): TestActiveInitialCheckpointS3ClientV1 = journalPublicationSdkCall {
            requireConnectionFree()
            ...
            val result = runCatching {
                ...
                reader.requireNativeRead()
                TestActiveInitialCheckpointS3ClientV1(reader, client, bounded, nanoTime).also { owner = it }
            }
    ...
    private companion object {
        const val PROFILE = "complaint-test-initial-checkpoint-s3-v1"

        fun overrides(call: TestActiveInitialCheckpointS3CallV1): AwsRequestOverrideConfiguration {
