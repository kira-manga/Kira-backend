...
internal class CatalogGenesisMutationInput private constructor(
    ...
    companion object {
        fun complete(readback: CatalogDualLocationVerifier.GenesisReadback): CatalogGenesisMutationInput {
            requireConnectionFree()
            requireCatalogReadback(readback.resume == GenesisResume.PREPARED, CatalogReadbackFailure.INVALID_LOCAL_STATE)
            return finalization(readback, PersistencePhasePath.COMPLAINT_CATALOG_GENESIS_COMPLETE)
        }

        fun project(readback: CatalogDualLocationVerifier.GenesisReadback): CatalogGenesisMutationInput {
            requireConnectionFree()
            return finalization(readback, PersistencePhasePath.COMPLAINT_CATALOG_GENESIS_PROJECT)
        }

        private fun finalization(readback: CatalogDualLocationVerifier.GenesisReadback, path: PersistencePhasePath): CatalogGenesisMutationInput =
            CatalogGenesisMutationInput(
                path,
                readback.mutation(),
                readback.mutation(),
                readback.manifest(),
                CatalogGenesisFinalizationInput.verified(readback),
            )
