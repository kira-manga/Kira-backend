 ...
 * membership. The separately approved terminal grant is committed into full D before activation/registration.
 * It cannot be added to an old accepted activation or prove an installed deny by syntax alone.
 * Neither a catalog key nor a candidate approval can construct or substitute this retained pin.
 */
internal class TestTerminalDenialAuthorityPolicyV1 private constructor(
    private val declaration: TestTerminalDenialAuthorityInputV1,
    private val key: RSAPublicKey,
) {
    private val canonical = CanonicalJson.canonicalize(TestTerminalDenialAuthorityInputV1.serializer(), declaration).toByteArray(Charsets.UTF_8)
    ...
    internal fun admit(
        original: TestRunTerminalQuiescenceV1,
        approval: ByteArray,
        rawEvidence: List<ByteArray>,
    ): AdmittedTerminalDenialV1 {
    ...
    ): AdmittedCatalogTerminalDenialV1 {
        requireConnectionFree()
        original.requireTerminalAuthority(this)
        val (body, evidence) = verify(approval, rawEvidence)
        original.requireTerminalDenialContext(body, evidence)
        original.requireTerminalAuthority(this)
    ...
    ): AdmittedErasureTerminalDenialV1 {
        requireConnectionFree()
        original.requireTerminalAuthority(this)
        val (body, evidence) = verify(approval, rawEvidence)
        original.requireTerminalDenialContext(body, evidence)
    ...
    private fun verify(approval: ByteArray, rawEvidence: List<ByteArray>): Pair<TestTerminalDenialStatementV1, TestTerminalEvidenceDigestV1> {
        ...
        try {
            ...
            require(!iterator.hasNext()) { REFUSAL }
            val envelope = OfflineTrustBundleParser.parseTerminalDenial(input)
            ...
            val canonicalBody = CanonicalJson.canonicalize(TestTerminalDenialStatementV1.serializer(), body).toByteArray(Charsets.UTF_8)
    ...
    override fun toString(): String = "TestTerminalDenialAuthorityPolicyV1(independent-purpose-pin,no-provenance-claim)"
    ...
    private class Admitted(
        ...
        private val policy: TestTerminalDenialAuthorityPolicyV1,
        override val statement: TestTerminalDenialStatementV1,
        override val policyEvidence: TestTerminalEvidenceDigestV1,
    ) : AdmittedTerminalDenialV1 {
        override fun requireOriginal(candidate: TestRunTerminalQuiescenceV1) {
        ...
        override fun toString(): String = "AdmittedTerminalDenialV1(original-only,redacted)"
    }
    ...
    private class CatalogAdmitted(
        ...
        override fun requireOriginal(candidate: CatalogTestRunTerminalV1) {
            require(candidate === original) { REFUSAL }
            original.requireTerminalAuthority(policy)
        }
        override fun firstStartAfter(): Instant = Instant.ofEpochSecond(Math.addExact(Math.addExact(
            maxOf(statement.denialEffectiveAtEpochSecond, statement.lastSessionExpiryEpochSecond), statement.acceptedRequestBoundSeconds),
            Math.multiplyExact(2L, statement.utcUncertaintySeconds)))
        override fun toString(): String = "AdmittedCatalogTerminalDenialV1(original-only,redacted)"
    }
    ...
    private class ErasureAdmitted(
        ...
        override fun firstStartAfter(): Instant = Instant.ofEpochSecond(Math.addExact(Math.addExact(
            maxOf(statement.denialEffectiveAtEpochSecond, statement.lastSessionExpiryEpochSecond), statement.acceptedRequestBoundSeconds),
            Math.multiplyExact(2L, statement.utcUncertaintySeconds)))
        override fun toString(): String = "AdmittedErasureTerminalDenialV1(original-only,redacted)"
    }

    companion object {
        const val PROFILE = "TEST_TERMINAL_DENIAL_AUTHORITY_V1"
        const val PURPOSE = "TEST_SEAL_TERMINAL_PUT_DENIAL_AND_REQUEST_BOUND_V1"
        const val RUN_ENVELOPE_ACCOUNTING = "REUSE_FIRST_CUT_LIFETIME_ENVELOPE_V1"
        ...
        private const val REFUSAL = "TEST terminal denial authority refused."
        private val REFERENCE = Regex("[A-Za-z0-9._:/-]{1,128}")

        fun fromIndependentInput(input: TestTerminalDenialAuthorityInputV1): TestTerminalDenialAuthorityPolicyV1 {
            requireConnectionFree()
            ...
            return try {
                ...
                TestTerminalDenialAuthorityPolicyV1(input.copy(), OfflineTrustBundleCrypto.publicKey(spki))
            } finally { spki.fill(0) }
        }

        internal fun frame(keyId: String, canonicalBody: ByteArray): ByteArray {
            val fields = listOf("kira.complaints.test-terminal-denial.v1".toByteArray(), "kcj-1".toByteArray(), keyId.toByteArray(),
        ...
        private fun validateTimes(body: TestTerminalDenialStatementV1) {
...
internal sealed interface AdmittedTerminalDenialV1 {
    val statement: TestTerminalDenialStatementV1
    val policyEvidence: TestTerminalEvidenceDigestV1
    fun requireOriginal(candidate: TestRunTerminalQuiescenceV1)
...
internal sealed interface AdmittedCatalogTerminalDenialV1 {
    val statement: TestTerminalDenialStatementV1
    val policyEvidence: TestTerminalEvidenceDigestV1
...
internal sealed interface AdmittedErasureTerminalDenialV1 {
    val statement: TestTerminalDenialStatementV1
    val policyEvidence: TestTerminalEvidenceDigestV1
