...
/** Closed TEST manifest facade; frozen original-holder release is checked on every wire call; raw HTTP bounds, XML inspection, observations and native cleanup are shared. */
internal class TestInstallationManifestS3ClientV1 private constructor(
    ...
    fun listExact(binding: TestInstallationManifestS3BindingV1): JournalListedVersionV1? {
        val call = TestInstallationManifestS3CallV1.list(binding, nanoTime)
    ...
    fun putIfAbsent(binding: TestInstallationManifestS3BindingV1, candidate: TestInstallationManifestS3CandidateV1): JournalPutObservationV1 {
        val call = TestInstallationManifestS3CallV1.put(binding, candidate, nanoTime)
        val location = call.declaration.journalLocation
        return execute(call) {
    ...
    fun getVersion(binding: TestInstallationManifestS3BindingV1, versionId: String): JournalFetchedVersionV1 {
        val call = TestInstallationManifestS3CallV1.get(binding, versionId, nanoTime)
        val location = call.declaration.journalLocation
    ...
    private fun <T> execute(call: TestInstallationManifestS3CallV1, action: () -> T): T = journalPublicationSdkCall {
    ...
    internal class Construction : AutoCloseable {
        ...
        private var owner: TestInstallationManifestS3ClientV1? = null
        ...
        fun open(routing: TestOwnerDeleteJournalRoutingV1, credentials: AwsSessionCredentials, httpFactory: (remainingMillis: () -> Int) -> SdkHttpClient,
            nanoTime: () -> Long, attempt: TestTerminalAttemptV1): TestInstallationManifestS3ClientV1 = journalPublicationSdkCall {
            requireConnectionFree()
            requireJournalPublication(!opened && !closed)
            opened = true
            val result = runCatching {
                attempt.requireJournal(routing.journalConfiguration); attempt.remainingMillis(1)
                ...
                attempt.remainingMillis(1)
                TestInstallationManifestS3ClientV1(routing, client, bounded, nanoTime).also { owner = it }
            }
    ...
    private companion object {
        const val PROFILE = "complaint-test-installation-manifest-s3-v1"
        fun overrides(call: TestInstallationManifestS3CallV1): AwsRequestOverrideConfiguration {
