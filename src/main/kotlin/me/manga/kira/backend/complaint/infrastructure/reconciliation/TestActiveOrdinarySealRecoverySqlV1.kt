...
/** Fixed EMPTY pre-VERIFY reads/current lease, missing-wire CAS and VERIFY only. No charge or canonical writer. */
internal object TestActiveOrdinarySealRecoverySqlV1 {
    ...
    val current = """
        ...
        SELECT (e.scope = d.scope AND r.test_only AND r.state = 'ACTIVE' AND (${ComplaintInstallationTestRunRows.activeShape})
            ...
            AND complaint_is_v4(i.operation_token) AND complaint_is_v4(i.request_owner) AND i.request_token > 0
            AND complaint_is_v4(i.capture_owner) AND i.capture_token >= i.request_token
            AND i.state IN ('CANONICAL', 'WIRE_FROZEN')
            ...
            sha256(convert_to((to_jsonb(c) - ARRAY['lease_owner','lease_token','lease_expires_at','updated_at',
                'seal_state','seal_object_version','seal_ciphertext_hash','seal_retain_until','seal_verified_at',
                'seal_verification_bytes','seal_verification_hash'])::text, 'UTF8')) AS control_static,
            sha256(convert_to((to_jsonb(i) - ARRAY['state','wire_bytes','wire_hash','checksum_sha256','content_type',
                'object_lock_mode','retain_until','metadata_bytes','metadata_hash','frozen_at'])::text, 'UTF8')) AS paid_static,
    ...
    // Run is already locked; this recovery read authenticates its full current ACTIVE identity.
    val runAccounting = """
    ...
    // Unfiltered LIMIT-1 probes: malformed families/states/keys cannot disappear behind a success filter.
    val epochPresent = "SELECT 1 FROM complaint_journal_publications WHERE data_scope_id = ?::uuid AND writer_generation = ?::uuid LIMIT 1"
    val writerBefore = TestActiveCutoffPublicationSqlV1.writerBefore
    val writerAfter = TestActiveCutoffPublicationSqlV1.writerAfter
    val keyPresent = "SELECT 1 FROM complaint_journal_publications WHERE object_key COLLATE \"C\" >= ?::text COLLATE \"C\" AND object_key COLLATE \"C\" < ?::text COLLATE \"C\" LIMIT 1"
}
