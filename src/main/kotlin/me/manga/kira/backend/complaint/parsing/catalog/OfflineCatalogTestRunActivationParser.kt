...
/** Syntax/canonical checks only, never a full-D, signature, accepted-run or provider-evidence factory. */
internal object OfflineCatalogTestRunActivationParser {
    ...
    private const val RUN_PATH = "manifest.activationRecord.run"
    ...
    fun parse(
        ...
        maximumDocumentBytes: Int = OfflineCatalogTestRunActivationProtocol.MAX_DOCUMENT_BYTES,
    ): OfflineCatalogTestRunActivationEnvelopeV3 = checked {
        ...
        val envelope = json.decodeFromString(OfflineCatalogTestRunActivationEnvelopeV3.serializer(), text)
        requireOfflineTrustBundle(envelope.schemaVersion == OfflineCatalogTestRunActivationProtocol.SCHEMA_VERSION, OfflineTrustBundleFailure.UNSUPPORTED_SCHEMA)
        OfflineCatalogTestRunActivationSyntaxV3.validateManifest(envelope.manifest)
        requireOfflineTrustBundle(envelope.signatures.size == 1)
        val canonical = CanonicalJson.canonicalize(OfflineCatalogTestRunActivationEnvelopeV3.serializer(), envelope).toByteArray(Charsets.UTF_8)
    ...
    fun parseManifest(
        ...
        maximumManifestRecords: Int,
        maximumDocumentBytes: Int = OfflineCatalogTestRunActivationProtocol.MAX_DOCUMENT_BYTES,
    ): OfflineCatalogTestRunActivationManifestV3 = checked {
        val input = snapshotDocument(bytes, maximumManifestRecords, maximumDocumentBytes)
        val text = decodeUtf8(input)
        validateStructure(text, maximumManifestRecords, rootPath = "manifest")
        val manifest = json.decodeFromString(OfflineCatalogTestRunActivationManifestV3.serializer(), text)
        OfflineCatalogTestRunActivationSyntaxV3.validateManifest(manifest)
        val canonical = CanonicalJson.canonicalize(OfflineCatalogTestRunActivationManifestV3.serializer(), manifest).toByteArray(Charsets.UTF_8)
        requireOfflineTrustBundle(canonical.contentEquals(input), OfflineTrustBundleFailure.NON_CANONICAL)
        manifest
    }

    /** Dispatch only. Prefixes still use their original parser and bounds, not schema3's smaller cap/arrays. */
    fun parseGeneration(
        bytes: ByteArray,
        maximumManifestRecords: Int,
        maximumEnvelopeBytes: Int = OfflineCatalogChainProtocol.MAX_ENVELOPE_BYTES,
    ): ParsedOfflineTestRunCatalogGeneration = checked {
        ...
        when (schemaVersion(decodeUtf8(input))) {
            1L, 2L -> ParsedOfflineTestRunCatalogGeneration.Prefix(OfflineCatalogInventoryParser.parse(input, maximumManifestRecords))
            3L -> ParsedOfflineTestRunCatalogGeneration.ActivationV3(
                parse(input, maximumManifestRecords, minOf(maximumEnvelopeBytes, OfflineCatalogTestRunActivationProtocol.MAX_DOCUMENT_BYTES)),
            )
    ...
    private fun snapshotDocument(bytes: ByteArray, maximumManifestRecords: Int, maximumDocumentBytes: Int): ByteArray {
        requireOfflineTrustBundle(maximumManifestRecords in 1..OfflineCatalogChainProtocol.MAX_MANIFEST_RECORDS, OfflineTrustBundleFailure.INVALID_POLICY)
        requireOfflineTrustBundle(maximumDocumentBytes in 1..OfflineCatalogTestRunActivationProtocol.MAX_DOCUMENT_BYTES, OfflineTrustBundleFailure.INVALID_POLICY)
    ...
    /** All objects inside the manifest, including its root and embedded J, count toward the record ceiling. */
    ...
    private fun arrayRule(path: String, maximumManifestRecords: Int): ArrayRule = when (path) {
        ...
        "$RUN_PATH.journalConfiguration.routingKeys" -> ArrayRule(1, 4, JsonToken.START_OBJECT)
        "$RUN_PATH.noticeSeeds" -> ArrayRule(OfflineCatalogTestRunActivationProtocol.NOTICE_COUNT, OfflineCatalogTestRunActivationProtocol.NOTICE_COUNT, JsonToken.START_OBJECT)
        "$RUN_PATH.accounting.activationCatalogPrepareActual",
        "$RUN_PATH.accounting.activationProjectionActual",
        "$RUN_PATH.accounting.originalUnusedReserve" -> ArrayRule(ComplaintCapacityEncoding.WIDTH, ComplaintCapacityEncoding.WIDTH, JsonToken.VALUE_NUMBER_INT)
...
internal sealed interface ParsedOfflineTestRunCatalogGeneration {
    data class Prefix(val generation: ParsedOfflineCatalogGeneration) : ParsedOfflineTestRunCatalogGeneration
    data class ActivationV3(val envelope: OfflineCatalogTestRunActivationEnvelopeV3) : ParsedOfflineTestRunCatalogGeneration
}
