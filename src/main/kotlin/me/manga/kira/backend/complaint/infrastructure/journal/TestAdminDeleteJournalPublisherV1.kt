...
internal class TestAdminDeleteJournalPublisherV1 private constructor(
    ...
    internal fun publish(work: CommittedTestAdminDeleteWork.Prepared, attempt: TestOwnerDeleteCodecAttemptV1): TestOwnerDeleteJournalReadbackV1 =
    ...
    internal fun readExisting(tuple: TestAdminDeleteJournalTupleV1, targetId: UUID, routingKeyId: String, attempt: TestOwnerDeleteCodecAttemptV1): TestOwnerDeleteJournalReadbackV1 =
        journalPublicationCall(JournalPublicationFailureV1.INVALID_BINDING) {
            requireConnectionFree()
            requireJournalPublication(tuple.eventKind == ComplaintJournalDeletionKindV1.ADMIN_DELETE)
            requireJournalPublication(!closed.get() && used.compareAndSet(false, true))
            val expected = codec.canonicalizeAdmin(tuple, targetId, routingKeyId)
    ...
    internal class Construction : AutoCloseable {
        ...
        private var owner: TestAdminDeleteJournalPublisherV1? = null
        private var failure: Throwable? = null
        internal fun open(factory: TestAdminDeleteJournalPublisherFactoryV1, lane: JournalPublicationLanesV1.TestAdminDeleteReservation,
            store: JdbcComplaintAdminDeleteStore, routing: TestOwnerDeleteJournalRoutingV1, credentials: AwsSessionCredentials,
            s3Http: (remainingMillis: () -> Int) -> SdkHttpClient, kmsHttp: (remainingMillis: () -> Int) -> SdkHttpClient,
            clock: Clock, nanoTime: () -> Long, attempt: TestOwnerDeleteCodecAttemptV1): TestAdminDeleteJournalPublisherV1 =
            journalPublicationCall(JournalPublicationFailureV1.INVALID_BINDING) {
                requireConnectionFree()
                ...
                val result = runCatching {
                    ...
                    lane.requireConstructing(factory, this, attempt)
                    TestAdminDeleteJournalPublisherV1(store, routing, TestOwnerDeleteJournalCodecV1(routing, dataKeys, nanoTime = nanoTime), dataKeys, client,
