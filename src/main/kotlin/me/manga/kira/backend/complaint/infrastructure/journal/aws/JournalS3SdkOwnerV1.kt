...
/** Shared SDK mechanics with one fixed ordinary family or exact lower seal binding per owner. */
internal class JournalS3SdkOwnerV1 private constructor(
    private val routing: VersionBoundComplaintJournalRouting,
    private val epochSealBinding: EpochSealS3BindingV1?,
    ...
    fun listExact(binding: JournalS3BindingV1): JournalListedVersionV1? = list(JournalS3CallV1.list(binding, nanoTime))

    fun listEpochSeal(binding: EpochSealS3BindingV1): JournalListedVersionV1? = list(EpochSealS3CallV1.list(binding, nanoTime))

    private fun list(call: JournalS3RequestV1): JournalListedVersionV1? {
        val response = execute(call) {
            sdk.listObjectVersions(
                ListObjectVersionsRequest.builder().bucket(call.declaration.journalLocation.bucket)
                    .expectedBucketOwner(call.declaration.journalLocation.accountId).prefix(call.objectKey)
    ...
    fun putIfAbsent(binding: JournalS3BindingV1, candidate: JournalS3CandidateV1): JournalPutObservationV1 =
        put(JournalS3CallV1.put(binding, candidate, nanoTime))

    fun putEpochSeal(binding: EpochSealS3BindingV1): JournalPutObservationV1 = put(EpochSealS3CallV1.put(binding, nanoTime))

    private fun put(call: JournalS3RequestV1): JournalPutObservationV1 {
                    ...
                    journalPublicationSdkCall {
                        sdk.putObject(
                            PutObjectRequest.builder().bucket(call.declaration.journalLocation.bucket)
                                .expectedBucketOwner(call.declaration.journalLocation.accountId).key(call.objectKey)
    ...
    fun getVersion(binding: JournalS3BindingV1, versionId: String): JournalFetchedVersionV1 = get(JournalS3CallV1.get(binding, versionId, nanoTime))

    fun getEpochSeal(binding: EpochSealS3BindingV1, versionId: String): JournalFetchedVersionV1 = get(EpochSealS3CallV1.get(binding, versionId, nanoTime))

    private fun get(call: JournalS3RequestV1): JournalFetchedVersionV1 {
            ...
            execute(call) {
                val response = sdk.getObjectAsBytes(
                    GetObjectRequest.builder().bucket(call.declaration.journalLocation.bucket)
                        .expectedBucketOwner(call.declaration.journalLocation.accountId).key(call.objectKey).versionId(call.versionId)
    ...
    private fun inventory(response: ListObjectVersionsResponse, call: JournalS3RequestV1): JournalListedVersionV1? {
        ...
        requireJournalPublication(
            response.name() == call.declaration.journalLocation.bucket && response.prefix() == call.objectKey &&
        ...
        versions.forEach {
            requireJournalPublication(it.key() == call.objectKey && it.isLatest() == true, JournalPublicationFailureV1.INVALID_LISTING)
    ...
    private fun uncertainOrConflict(call: JournalS3RequestV1, failure: Throwable): JournalPutObservationV1 {
    ...
    private fun <T> execute(call: JournalS3RequestV1, action: () -> T): T = journalPublicationSdkCall {
        requireConnectionFree()
        requireJournalPublication(call.routing === routing && accepts(call) && !closed.get() && busy.compareAndSet(false, true))
    ...
    private fun accepts(call: JournalS3RequestV1): Boolean = when (call) {
        is JournalS3CallV1 -> epochSealBinding == null
        is EpochSealS3CallV1 -> epochSealBinding === call.binding
    }
    ...
    override fun toString(): String = "JournalS3SdkOwnerV1(fixed-family,J-bound,redacted)"
    ...
    internal class Construction : AutoCloseable {
        ...
        ): JournalS3SdkOwnerV1 = openProfile(routing, credentials, httpFactory, nanoTime, attempt, null)

        /** Explicit lower fixture construction only; no sealer-role provenance or production entry is inferred. */
        internal fun openEpochSealFixture(
            binding: EpochSealS3BindingV1,
            fixtureCredentials: AwsSessionCredentials,
            httpFactory: () -> SdkHttpClient,
            nanoTime: () -> Long,
        ): JournalS3SdkOwnerV1 = openProfile(binding.routing, fixtureCredentials, { httpFactory() }, nanoTime, null, binding)
        ...
        private fun openProfile(
            ...
            nanoTime: () -> Long,
            attempt: JournalCodecAttemptV1?,
            sealBinding: EpochSealS3BindingV1?,
        ): JournalS3SdkOwnerV1 = journalPublicationSdkCall {
            requireConnectionFree()
            ...
            val result = runCatching {
                ...
                val profileName = if (sealBinding == null) ORDINARY_PROFILE_NAME else SEAL_PROFILE_NAME
                val endpoint = regionalEndpoint(checkNotNull(region), emptyProfile, profileName)
                attempt?.remainingMillis(1)
                sealBinding?.check()
                ...
                val sdk = S3Client.builder().region(region).credentialsProvider(StaticCredentialsProvider.create(credentials))
                    ...
                    .serviceConfiguration(
                        S3Configuration.builder().pathStyleAccessEnabled(true).accelerateModeEnabled(false).chunkedEncodingEnabled(false)
                            ...
                            .profileFile(emptyProfile).profileName(profileName).build(),
                    )
                    ...
                    .overrideConfiguration(
                        ClientOverrideConfiguration.builder().defaultProfileFile(emptyProfile).defaultProfileName(profileName)
                ...
                attempt?.remainingMillis(1)
                sealBinding?.check()
                JournalS3SdkOwnerV1(routing, sealBinding, sdk, transport, nanoTime).also { owner = it }
            }
    ...
    companion object {
        private fun overrides(call: JournalS3RequestV1): AwsRequestOverrideConfiguration {
        ...
        private fun regionalEndpoint(region: Region, emptyProfile: ProfileFile, profileName: String): URI {
            ...
            val metadata = S3Client.serviceMetadata().reconfigure(
                ServiceMetadataConfiguration.builder().profileFile { emptyProfile }.profileName(profileName)
        ...
        private const val ORDINARY_PROFILE_NAME = "complaint-journal-ordinary-v1"
        private const val SEAL_PROFILE_NAME = "complaint-epoch-seal-s3-v1"
    }
}
