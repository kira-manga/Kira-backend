...
/** One fixed, UNREGISTERED TEST-only atomic STATUS batch. No issuer, sourceOnly or activation bypass. */
internal class ComplaintAdminBatchStatusHttpHandler(
    ...
    private fun exchange(request: HttpServletRequest, response: DeliveryResponse, context: ComplaintIngressContext) {
        ...
        val input = try {
            ...
            ComplaintAdminBatchStatusParser.parse(bytes, scope, headers.key)
        } finally {
            bytes.fill(0)
        }
    ...
    override fun toString(): String = "ComplaintAdminBatchStatusHttpHandler(unregistered,TEST-only)"
