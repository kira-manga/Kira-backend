...
import me.manga.kira.backend.complaint.infrastructure.journal.aws.TestRunPurgeS3BindingV1
import me.manga.kira.backend.complaint.infrastructure.journal.aws.TestRunPurgeS3CandidateV1
...
internal class TestRunPurgeProofV1 private constructor(
    private val custody: TestRunPurgeCustodyV1,
    val version: String,
    val lastModified: Instant,
    val retainUntil: Instant,
    val verifiedAt: Instant,
) {
    internal fun requireOriginal(original: TestRunPurgePublicationV1) { custody.requireReleasedProof(original, this) }
    ...
    internal fun canonicalBytes(row: TestTerminalDurableRowV1, at: Instant = verifiedAt): ByteArray {
        requirePurge(at.nano % 1000 == 0 && !at.isBefore(lastModified) && !at.isAfter(verifiedAt) && retainUntil.isAfter(at))
        return CanonicalJson.canonicalize(buildJsonObject {
            put("schemaVersion", 1); put("objectKind", "TEST_RUN_PURGE"); put("objectOrdinal", row.binding.objectOrdinal)
    ...
    override fun toString(): String = "TestRunPurgeProofV1(authenticated-exact-version,purge-publication-only,redacted)"

    companion object {
        internal fun publish(custody: TestRunPurgeCustodyV1, s3: TestRunPurgeS3ClientV1, keys: AwsTestTerminalDataKeyAdapterV1): TestRunPurgeProofV1 {
            custody.requirePublication()
            val binding = TestRunPurgeS3BindingV1.released(custody)
            ...
            if (listed == null) {
                ...
                val candidate = TestRunPurgeS3CandidateV1.frozen(binding)
            ...
            acknowledgment?.let { (put, _) -> requirePurge(put.versionId == exact.versionId && put.wireSha256 == frozen.wireSha256) }
            ...
            return withJournalPublicationCleanup({
                ...
                acknowledgment?.let { (_, lock) -> requirePurge(!facts.second.isBefore(lock)) }
                custody.requirePublication()
                ...
                requirePurge(decoded.content === custody.content() && decoded.wireSha256 == frozen.wireSha256)
                ...
                custody.requirePublication()
                TestRunPurgeProofV1(custody, exact.versionId, facts.first, facts.second, now)
            }, fetched::close)
        }
        ...
        private fun cheapChecks(row: TestTerminalDurableRowV1, listed: JournalListedVersionV1, fetched: JournalFetchedVersionV1): Pair<Instant, Instant> {
            ...
            requirePurge(raw.statusCode() == 200 && response.sdkHttpResponse().statusCode() == 200 &&
            ...
            try { requirePurge(MessageDigest.isEqual(wire, fetched.bytes)) } finally { wire.fill(0) }
            requirePurge(response.contentLength() == listed.size && fetched.bytes.size.toLong() == listed.size &&
            ...
            requirePurge(response.deleteMarker() != true && response.contentRange() == null && response.contentEncoding() == null && response.expiration() == null)
            requirePurge(response.contentType() == JournalS3HttpWireV1.CONTENT_TYPE && JournalS3HttpWireV1.single(headers, "Content-Type") == JournalS3HttpWireV1.CONTENT_TYPE)
            requirePurge(response.metadata() == row.metadata() && JournalS3HttpWireV1.metadata(headers) == row.metadata() && (response.missingMeta() == null || response.missingMeta() == 0))
            val checksum = JournalS3HttpWireV1.single(headers, "x-amz-checksum-sha256")
            requirePurge(checksum == row.checksumSha256 && response.checksumSHA256() == checksum &&
            ...
            try { requirePurge(Base64.getEncoder().encodeToString(hash) == checksum) } finally { hash.fill(0) }
            requirePurge(response.objectLockModeAsString() == "COMPLIANCE" && JournalS3HttpWireV1.single(headers, "x-amz-object-lock-mode") == "COMPLIANCE")
            val created = checkNotNull(response.lastModified())
            val retained = checkNotNull(response.objectLockRetainUntilDate())
            requirePurge(created == listed.lastModified &&
