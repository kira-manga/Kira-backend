...
internal class ComplaintAdmissionCharge(val key: ComplaintAdmissionBucketKey, val limit: Int)
...
internal class ComplaintAdmissionWindowStore(
    ...
    fun charge(charges: List<ComplaintAdmissionCharge>, now: Long) {
        ...
        val expiresAt = Math.addExact(now, idleNanos)
        prune(now)
        var retryNanos = 0L
        charges.forEach { charge ->
            val bucket = buckets[charge.key]
            if (bucket != null) {
                check(bucket.limit == charge.limit)
                events -= bucket.prune(now, windowNanos)
                if (bucket.size == bucket.limit) retryNanos = maxOf(retryNanos, windowNanos - (now - bucket.first()))
            }
        }
        if (retryNanos > 0) {
            // Idle means last observed ingress attempt, including an exhausted existing bucket.
            if (idleNanos > windowNanos) charges.forEach { charge -> buckets[charge.key]?.let { touch(charge.key, it, expiresAt) } }
        ...
        val selected = charges.map { charge -> charge to (buckets[charge.key] ?: Bucket(charge.limit)) }
        selected.forEach { (charge, bucket) ->
            buckets[charge.key] = bucket
            bucket.add(now)
            events += 1
            touch(charge.key, bucket, expiresAt)
        }
    }
    ...
    private class Bucket(val limit: Int) {
