...
internal class AwsJournalDataKeyAdapter private constructor(
    ...
    override fun generate(request: JournalDataKeyRequestV1): JournalGeneratedDataKeyV1 =
        acquire(request, JournalKmsOperation.GENERATE, null, ::generateKey)
    ...
    private inner class GeneratedLease(arn: String, key: ByteArray, override val wrappedKey: ByteArray) : KeyLease(arn, key),
        ...
        private fun create(
            ...
            return journalKmsSdkCall {
                ...
                val transport = BoundedJournalKmsSdkHttpClient(
                    profile.region, endpoint, credentials.accessKeyId(), credentials.sessionToken(), httpFactory,
                )
