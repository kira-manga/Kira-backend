...
/**
 * Dormant actual SDK adapter for one retained TEST J/attempt's fixed terminal kind. No key
 ...
 */
internal class AwsTestTerminalDataKeyAdapterV1 private constructor(
    val journal: TestOwnerDeleteJournalConfigurationV1,
    private val profile: TestTerminalKmsRequestProfileV1,
    private val sdk: KmsClient,
    private val transport: BoundedJournalKmsSdkHttpClient,
    override val attempt: TestTerminalAttemptV1,
) : TestTerminalDataKeyPortV1,
    AutoCloseable {
    ...
    private fun <T : KeyLease> acquire(
        ...
        val started = journalKmsSdkCall {
            attempt.requireJournal(journal)
            attempt.remainingProviderMillis(1)
            attempt.providerNanoTime()
        }
        ...
        val result = runCatching {
            journalKmsSdkCall {
                val call = profile.prepare(request, operation, wrapped, started).also { ownedCall = it }
                call.check()
    ...
    override fun toString(): String = "AwsTestTerminalDataKeyAdapterV1(TEST,J-attempt-bound,redacted,no-activation-authority)"
    ...
    internal class Construction : AutoCloseable {
        ...
        private var owner: AwsTestTerminalDataKeyAdapterV1? = null
        ...
        internal fun open(
        ...
        ): AwsTestTerminalDataKeyAdapterV1 {
            requireConnectionFree()
            return journalKmsSdkCall {
                ...
                val result = runCatching {
                    attempt.requireJournal(journal)
                    attempt.remainingProviderMillis(1)
                    val profile = TestTerminalKmsRequestProfileV1(journal, attempt)
                    ...
                    val constructionMillis = attempt.remainingProviderMillis(profile.callLimitMillis)
                    ...
                    val sdk = KmsClient.builder().region(region).credentialsProvider(StaticCredentialsProvider.create(credentials))
                        ...
                        .httpClient(transport).overrideConfiguration(
                            ClientOverrideConfiguration.builder().defaultProfileFile(emptyProfile).defaultProfileName(PROFILE_NAME)
                                ...
                                .apiCallTimeout(Duration.ofMillis(constructionMillis.toLong()))
                                .apiCallAttemptTimeout(Duration.ofMillis(constructionMillis.toLong())).build(),
                        ).build().also {
                            this.sdk = it
                            stage = ConstructionStage.SDK_RETURNED
                        }
                    attempt.remainingProviderMillis(1)
                    val adapter = AwsTestTerminalDataKeyAdapterV1(journal, profile, sdk, transport, attempt).also { owner = it }
                    attempt.remainingProviderMillis(1)
                    adapter
                }
        ...
        override fun toString(): String = "TestTerminalKmsConstruction(concrete,redacted)"
    ...
    companion object {
        fun open(
            ...
            credentials: AwsSessionCredentials,
            attempt: TestTerminalAttemptV1,
        ): AwsTestTerminalDataKeyAdapterV1 = Construction().open(journal, credentials, attempt, ::journalKmsUrlConnectionClient)
        ...
        ): AwsTestTerminalDataKeyAdapterV1 = Construction().open(journal, credentials, attempt, { httpFactory() })
        ...
        private const val PROFILE_NAME = "complaint-test-terminal-data-key-v1"
    }
}
