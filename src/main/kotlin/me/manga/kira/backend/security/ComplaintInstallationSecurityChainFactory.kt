...
internal class ComplaintInstallationSecurityChainFactory private constructor(
    ...
    private val initialCompleteOwner: InitialCompleteOwner? = null,
) {
    ...
    init {
        ...
        require(initialCompleteOwner == null || (core == null && initialCreate == null && initialOwnerDelete == null))
        initialCreate?.let {
        ...
        initialCompleteOwner?.let {
            require(it.create.hasDeleteStatus() && it.create.usesDeleteStatus(it.delete) &&
                it.create.hasEditStatus() && it.create.usesEditStatus(it.edit)) { "Complaint complete owner subset refused." }
        }
        core?.let {
    ...
    private val authentication: ComplaintInstallationBearerAuthenticator? get() = core?.authentication ?: initialCreate?.authentication ?: initialOwnerDelete?.authentication ?: initialCompleteOwner?.authentication
    private val detail: ComplaintOwnerDetailHttpHandler? get() = core?.detail ?: initialCreate?.reads?.detail ?: initialOwnerDelete?.reads?.detail ?: initialCompleteOwner?.reads?.detail
    private val reply: ComplaintOwnerCreateHttpHandler? get() = core?.reply ?: initialCreate?.reply ?: initialCompleteOwner?.create
    private val edit: ComplaintOwnerEditHttpHandler? get() = core?.edit ?: initialCreate?.edit ?: initialCompleteOwner?.edit
    private val me: ComplaintInstallationMeHttpHandler? get() = core?.me ?: initialCreate?.me ?: initialCompleteOwner?.me
    private val delete: ComplaintOwnerDeleteHttpHandler? get() = core?.delete ?: initialOwnerDelete?.delete ?: initialCompleteOwner?.delete
    private val deleteAll: ComplaintOwnerDeleteAllHttpHandler? get() = core?.deleteAll ?: initialOwnerDelete?.deleteAll ?: initialCompleteOwner?.deleteAll
        ...
        if (!implemented(request)) {
            ...
            when (ComplaintInstallationRoutes.path(request)) {
                ...
                ComplaintInstallationRoutes.ENROLLMENT, ComplaintInstallationRoutes.SESSION ->
                    checkNotNull(core?.installations ?: initialCreate?.installations ?: initialOwnerDelete?.installations ?: initialCompleteOwner?.installations).handleWithinIngress(request, response, context)

                ComplaintInstallationRoutes.ME -> checkNotNull(me).handleWithinIngress(request, response, context)

                ComplaintInstallationRoutes.STATUS -> checkNotNull(core?.create ?: initialCreate?.create ?: initialOwnerDelete?.create ?: initialCompleteOwner?.create).handleWithinIngress(request, response, context)
                ...
                ComplaintInstallationRoutes.HISTORY -> if (request.method == "GET") {
                    checkNotNull(core?.history ?: initialCreate?.reads?.history ?: initialOwnerDelete?.reads?.history ?: initialCompleteOwner?.reads?.history).handleWithinIngress(request, response, context)
                } else {
                    checkNotNull(core?.create ?: initialCreate?.create ?: initialOwnerDelete?.create ?: initialCompleteOwner?.create).handleWithinIngress(request, response, context)
                }
    ...
    internal fun implemented(request: HttpServletRequest): Boolean = (core != null && ComplaintInstallationRoutes.implemented(request)) ||
        ((initialCreate != null || initialOwnerDelete != null || initialCompleteOwner != null) && request.method == "POST" && ComplaintInstallationRoutes.path(request) in INITIAL_CREATE_PATHS) ||
        ((initialCreate?.reads != null || initialOwnerDelete != null || initialCompleteOwner != null) && request.method == "GET" && ComplaintInstallationRoutes.path(request) == ComplaintInstallationRoutes.HISTORY) ||
    ...
    /** One all-required registered owner tuple. CREATE is also REPLY and carries these exact EDIT/DELETE status handlers. */
    private class InitialCompleteOwner(
        val authentication: ComplaintInstallationBearerAuthenticator,
        val installations: ComplaintInstallationHttpHandler,
        val create: ComplaintOwnerCreateHttpHandler,
        val reads: OwnerReads,
    ...
    companion object {
        ...
        /** Fixed coherent owner cohort only; no earlier subset or broad Core constructor changes. */
        fun registeredCompleteOwnerSubset(bridge: ComplaintHttpIngressBridge, bootstrap: ComplaintInstallationBootstrapHttpHandler,
            authentication: ComplaintInstallationBearerAuthenticator, installations: ComplaintInstallationHttpHandler,
            create: ComplaintOwnerCreateHttpHandler, history: ComplaintOwnerHistoryHttpHandler, detail: ComplaintOwnerDetailHttpHandler,
            me: ComplaintInstallationMeHttpHandler, edit: ComplaintOwnerEditHttpHandler, delete: ComplaintOwnerDeleteHttpHandler,
            deleteAll: ComplaintOwnerDeleteAllHttpHandler): ComplaintInstallationSecurityChainFactory =
            ComplaintInstallationSecurityChainFactory(bridge, null, bootstrap,
                initialCompleteOwner = InitialCompleteOwner(authentication, installations, create, OwnerReads(history, detail), me, edit, delete, deleteAll))
