...
class AuditService(private val audit: AuditRepository, private val currentUser: CurrentUser, private val clock: Clock) {
    ...
    internal fun recordAdminDelete(outcome: AdminDeleteAuditOutcome, allocation: ComplaintAuditAllocation, at: Instant) {
        val detailJson = encode(outcome.scalarDetails())
        requireComplaintAuditPayloadSize(detailJson)
        val counted = checkNotNull(audit as? CountedAdminDeleteAuditRepository)
        counted.recordAdminDelete(CountedAdminDeleteAuditEntry(outcome, detailJson, at), allocation)
    }
