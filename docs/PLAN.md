...
| `PUT /api/v1/admin/sources/{api}/operational-mode` | Password-step-up protected, idempotent quick control. Body `{mode:"enabled"|"disabled"|"under_maintenance"}`. It derives state from the current immutable revision plus server lifecycle, creates a new immutable source revision only when `siteState` changes, and materializes exactly one signed catalog revision for every real change. `STOPPED`, `ADULT_18_PLUS`, draft, withheld, retired, removed, and non-generic sources are deliberately excluded. | 200 (including no-op); 400; 401; 404; 409 |
