...
CLASSES = {PREFIX + 'TutorialMediaReconciliationIT': 10}
CHILD_SHA = '56b66cfe8799123c719eaf048f81c542e5e4129d71c490cae99a38396c2a3385'
PAYLOAD_SHA = '9099cef61561eda449d0d4c9f9d32d782f18e2a1bc53d9a75ca29b8e1ac8fbc6'
IDENTITY_POLICY = 'EXACT_PUBLISHED_RAW_AND_COMPLETE_ENTRY_PAYLOADS'
CANCELLED = False
...
class Batch:
    ...
    def validate(self):
        ...
        require(os.environ.get('GITHUB_REPOSITORY') == 'kira-manga/Kira-backend'
                and os.environ.get('GITHUB_REF') == 'refs/heads/verify/backend-5-media-pg-04'
    ...
    def preserve(self):
        ...
        require(binding['schema_version'] == 2 and binding['identity_policy'] == IDENTITY_POLICY
                and binding['status'] == 'PASS' and binding['sha256'] == binding['expected_sha256'] == PAYLOAD_SHA
                and all(value is True for value in binding['predicates'].values()),
        ...
        for config in ('compileClasspath', 'testCompileClasspath', 'testRuntimeClasspath', 'testWorkerClasspath'):
            ...
            require(receipt['schema_version'] == 3 and receipt['identity_policy'] == IDENTITY_POLICY
                    and receipt['configuration'] == config and receipt['status'] == 'PASS'
                    and receipt['expected_payloads_sha256'] == PAYLOAD_SHA and receipt['observed_engine_count'] == 2
                    and receipt['expected_engine_pair'] == binding['expected_published_pair']
            ...
            engine = receipt['artifacts'] if config == 'testWorkerClasspath' else [
                item for item in receipt['artifacts'] if item.get('group') == 'me.manga.kira.source']
            require(len(engine) == 2 and all(item['accepted'] is True and item['acceptance'] == IDENTITY_POLICY
                    and item['comparisons']['whole_jar_sha256'] is True
                    and item['comparisons']['complete_entry_payload_inventory'] is True
                    and all(value is True for value in item['predicates'].values()) for item in engine),
                    'Exact published raw pair AND complete payload inventory required: ' + config)
        self.xml_ok = True
    ...
    def finish(self):
        ...
        result = {'status': 'PASS' if passed else 'FAIL', 'backend_sha': self.request['backend_sha'],
                  ...
                  'carried_hosted': self.request['carried_hosted'],
                  'dependency_verification': 'strict requested; no comprehensive checksum manifest; locks/exact Engine coordinates retained; exact authorized published whole-JAR SHA pair AND complete entry payload inventories enforced'}
