...
class Batch:
    ...
    def validate(self):
        ...
        require(os.environ.get('GITHUB_REPOSITORY') == 'kira-manga/Kira-backend'
                and os.environ.get('GITHUB_REF') == 'refs/heads/verify/backend-5-media-pg-01'
        ...
        self.gradle_exit, _ = self.command(args, 'gradle', seconds=20 * 60,
                                           extra={'KIRA_PACKAGES_USER': self.user, 'KIRA_PACKAGES_READ_TOKEN': self.token}, sample=True)
        self.stop('immediate')
    ...
    def preserve(self):
        ...
        for config in ('compileClasspath', 'testCompileClasspath', 'testRuntimeClasspath'):
            require((self.reports / ('dependencies-' + config + '.json')).is_file(), 'Missing ordinary classpath/hash readback')
        self.xml_ok = True
    ...
    def finish(self):
        ...
        result = {'status': 'PASS' if passed else 'FAIL', 'backend_sha': self.request['backend_sha'],
                  ...
                  'dependency_verification': 'strict requested; no comprehensive checksum manifest; locks and explicit Engine JVM pins enforced'}
