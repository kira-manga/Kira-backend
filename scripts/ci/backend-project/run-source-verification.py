...
assert binding['profile'] == 'PUBLIC_SOURCE_RECONSTRUCTION_NONSHIPPING_APP29_FOCUSED_11'
...
assert len(set(selectors)) == len(selectors) == selection['expected_selected_methods'] == 11
assert selection['expected_total'] == 11
...
def inventory():
    ...
    assert len(current) == inventory_pin['file_count'] == 2440
