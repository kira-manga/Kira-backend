...
assert binding['profile'] == 'PUBLIC_SOURCE_RECONSTRUCTION_NONSHIPPING_APP29_JOINED_77'
...
assert len(set(selectors)) == len(selectors) == selection['expected_selected_methods'] == 75
assert selection['expected_total'] == 77
...
def inventory():
    ...
    assert len(current) == inventory_pin['file_count'] == 1961
...
if safe_files:
    try:
        ...
        assert sorted(cases) == sorted(wanted) and len(cases) == 77
