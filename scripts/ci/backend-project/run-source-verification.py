...
assert binding['profile'] == 'PUBLIC_SOURCE_RECONSTRUCTION_NONSHIPPING_TERMINAL_B_11'
...
assert len(set(selectors)) == len(selectors) == selection['expected_total'] == 11
...
if safe_files:
    try:
        ...
        assert sorted(cases) == sorted(wanted) and len(cases) == 11
        ...
        ledger['exact11_and_actual_compilation'] = True
    except BaseException as error:
        partial_diagnostics()
...
ledger['terminal_b11_pass'] = (ledger['build_exit'] == 0 and not ledger['errors'] and ledger['scratch_removed'] and
    not ledger.get('children', {}).get('forced', True) and not ledger.get('final_children', {}).get('forced', True) and
    not ledger['container_forced'] and ledger.get('exact11_and_actual_compilation', False))
emit('actual-cleanup-and-result.json', ledger)
print('Public nonshipping terminal_b11 verification:', 'FOCUSED_PASS' if ledger['terminal_b11_pass'] else 'NOT_ACCEPTED')
sys.exit(0 if ledger['terminal_b11_pass'] else 1)
