...
assert binding['profile'] == 'PUBLIC_SOURCE_RECONSTRUCTION_NONSHIPPING_APP29_FOCUSED_119'
...
assert len(set(selectors)) == len(selectors) == selection['expected_selected_methods'] == 119
assert selection['expected_total'] == 120
...
def inventory():
    ...
    assert len(current) == inventory_pin['file_count'] == 2122
...
if safe_files:
    try:
        ...
        assert sorted(cases) == sorted(wanted) and len(cases) == 120
