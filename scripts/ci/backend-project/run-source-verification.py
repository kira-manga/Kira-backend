...
assert binding['profile'] == 'PUBLIC_SOURCE_RECONSTRUCTION_NONSHIPPING_OWNER_DELETE_REMAINDER_5'
...
assert len(set(selectors)) == len(selectors) == selection['expected_total'] == 5
...
def inventory():
    ...
    assert len(current) == inventory_pin['file_count'] == 1856
...
CONTAINER_IMAGES = {'postgres:17.6-alpine', 'testcontainers/ryuk:0.12.0'}
...
def current_container_records():
    ...
    pattern = re.compile(r'tc\.(postgres:17\.6-alpine|testcontainers/ryuk:0\.12\.0) -- Container \1 is starting: ([0-9a-f]{64})(?=\r?$)', re.MULTILINE)
    for path in paths:
        if not path.is_file():
            continue
...
if safe_files:
    try:
        ...
        assert sorted(cases) == sorted(wanted) and len(cases) == 5
        ...
        ledger['exact5_and_actual_compilation'] = True
    except BaseException as error:
        partial_diagnostics()
...
ledger['owner_delete_remainder5_pass'] = (ledger['build_exit'] == 0 and not ledger['errors'] and ledger['scratch_removed'] and
    not ledger.get('children', {}).get('forced', True) and not ledger.get('final_children', {}).get('forced', True) and
    not ledger['container_forced'] and ledger.get('exact5_and_actual_compilation', False))
emit('actual-cleanup-and-result.json', ledger)
print('Public nonshipping owner_delete_remainder5 verification:', 'FOCUSED_PASS' if ledger['owner_delete_remainder5_pass'] else 'NOT_ACCEPTED')
sys.exit(0 if ledger['owner_delete_remainder5_pass'] else 1)
