...
assert binding['profile'] == 'PUBLIC_SOURCE_RECONSTRUCTION_NONSHIPPING_BASE_REMAINDER_4'
...
assert len(set(selectors)) == len(selectors) == selection['expected_total'] == 4
...
if safe_files:
    try:
        ...
        assert sorted(cases) == sorted(wanted) and len(cases) == 4
        ...
        ledger['exact4_and_actual_compilation'] = True
    except BaseException as error:
        partial_diagnostics()
...
ledger['base_remainder4_pass'] = (ledger['build_exit'] == 0 and not ledger['errors'] and ledger['scratch_removed'] and
    not ledger.get('children', {}).get('forced', True) and not ledger.get('final_children', {}).get('forced', True) and
    not ledger['container_forced'] and ledger.get('exact4_and_actual_compilation', False))
emit('actual-cleanup-and-result.json', ledger)
print('Public nonshipping base_remainder4 verification:', 'FOCUSED_PASS' if ledger['base_remainder4_pass'] else 'NOT_ACCEPTED')
sys.exit(0 if ledger['base_remainder4_pass'] else 1)
