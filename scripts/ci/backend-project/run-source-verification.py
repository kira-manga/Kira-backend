...
assert binding['profile'] == 'PUBLIC_SOURCE_RECONSTRUCTION_NONSHIPPING_APP29_FOCUSED_82'
...
assert len(set(selectors)) == len(selectors) == selection['expected_selected_methods'] == 82
assert selection['expected_total'] == 82
...
if safe_files:
    try:
        ...
        assert sorted(cases) == sorted(wanted) and len(cases) == 82
