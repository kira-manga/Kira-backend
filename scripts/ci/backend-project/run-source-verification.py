...
assert binding['profile'] == 'PUBLIC_SOURCE_RECONSTRUCTION_NONSHIPPING_OWNER_DELETE_17'
...
assert len(set(selectors)) == len(selectors) == selection['expected_total'] == 17
...
if safe_files:
    try:
        ...
        assert sorted(cases) == sorted(wanted) and len(cases) == 17
        ...
        ledger['exact17_and_actual_compilation'] = True
    except BaseException as error:
        partial_diagnostics()
...
ledger['owner_delete17_pass'] = (ledger['build_exit'] == 0 and not ledger['errors'] and ledger['scratch_removed'] and
    not ledger.get('children', {}).get('forced', True) and not ledger.get('final_children', {}).get('forced', True) and
    not ledger['container_forced'] and ledger.get('exact17_and_actual_compilation', False))
emit('actual-cleanup-and-result.json', ledger)
print('Public nonshipping owner_delete17 verification:', 'FOCUSED_PASS' if ledger['owner_delete17_pass'] else 'NOT_ACCEPTED')
sys.exit(0 if ledger['owner_delete17_pass'] else 1)
