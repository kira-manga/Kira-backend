...
assert binding['profile'] == 'PUBLIC_SOURCE_RECONSTRUCTION_NONSHIPPING_REMAINING43_A_26'
...
assert len(set(selectors)) == len(selectors) == selection['expected_total'] == 26
...
if safe_files:
    try:
        ...
        assert sorted(cases) == sorted(wanted) and len(cases) == 26
        ...
        ledger['exact26_and_actual_compilation'] = True
    except BaseException as error:
        partial_diagnostics()
...
ledger['remaining43_a26_pass'] = (ledger['build_exit'] == 0 and not ledger['errors'] and ledger['scratch_removed'] and
    not ledger.get('children', {}).get('forced', True) and not ledger.get('final_children', {}).get('forced', True) and
    not ledger['container_forced'] and ledger.get('exact26_and_actual_compilation', False))
emit('actual-cleanup-and-result.json', ledger)
print('Public nonshipping remaining43_a26 verification:', 'FOCUSED_PASS' if ledger['remaining43_a26_pass'] else 'NOT_ACCEPTED')
sys.exit(0 if ledger['remaining43_a26_pass'] else 1)
