...
assert binding['profile'] == 'PUBLIC_SOURCE_RECONSTRUCTION_NONSHIPPING_APP29_FOCUSED_36'
...
assert len(set(selectors)) == len(selectors) == selection['expected_selected_methods'] == 36
assert selection['expected_total'] == 147
...
def inventory():
    ...
    assert len(current) == inventory_pin['file_count'] == 2498
...
common = ['--no-daemon', '--no-parallel', '--max-workers=1', '--no-build-cache', '--no-configuration-cache', '--console=plain',
    ...
    '-I', str(control / 'runtime-repositories.init.gradle'), '-I', str(control / 'normal-runtime.init.gradle'), '-I', str(control / 'serial-test-classes.init.gradle'), '-x', 'jacocoTestReport']
...
if safe_files:
    try:
        ...
        assert sorted(cases) == sorted(wanted) and len(cases) == 147
