...
assert binding['profile'] == 'PUBLIC_SOURCE_RECONSTRUCTION_NONSHIPPING_APP29_TEST_OWNER_DELETE_ALL_32'
...
assert len(set(selectors)) == len(selectors) == selection['expected_selected_methods'] == 32
assert selection['expected_total'] == 32
...
if safe_files:
    try:
        ...
        assert sorted(cases) == sorted(wanted) and len(cases) == 32
