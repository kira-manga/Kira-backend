...
assert binding['profile'] == 'PUBLIC_SOURCE_RECONSTRUCTION_NONSHIPPING_APP29_FOCUSED_1'
...
assert len(set(selectors)) == len(selectors) == selection['expected_selected_methods'] == 1
assert selection['expected_total'] == 1
...
if safe_files:
    try:
        ...
        assert sorted(cases) == sorted(wanted) and len(cases) == 1
