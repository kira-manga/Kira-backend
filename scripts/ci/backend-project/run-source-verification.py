...
assert binding['profile'] == 'PUBLIC_SOURCE_RECONSTRUCTION_NONSHIPPING_APP29_MANIFEST_ADMIN_157'
...
assert len(set(selectors)) == len(selectors) == selection['expected_selected_methods'] == 157
assert selection['expected_total'] == 158
...
def inventory():
    ...
    assert len(current) == inventory_pin['file_count'] == 2069
...
if safe_files:
    try:
        ...
        assert sorted(cases) == sorted(wanted) and len(cases) == 158
