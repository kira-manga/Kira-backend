...
assert binding['profile'] == 'PUBLIC_SOURCE_RECONSTRUCTION_NONSHIPPING_APP29_DRAIN_BOOTSTRAP_ALL_STATS_99'
...
assert len(set(selectors)) == len(selectors) == selection['expected_selected_methods'] == 99
assert selection['expected_total'] == 100
...
def inventory():
    ...
    assert len(current) == inventory_pin['file_count'] == 2020
...
if safe_files:
    try:
        ...
        assert sorted(cases) == sorted(wanted) and len(cases) == 100
