...
assert binding['profile'] == 'PUBLIC_SOURCE_RECONSTRUCTION_NONSHIPPING_APP29_PROOF_INTAKE_41'
...
assert len(set(selectors)) == len(selectors) == selection['expected_selected_methods'] == 41
assert selection['expected_total'] == 41
...
def inventory():
    ...
    assert len(current) == inventory_pin['file_count'] == 1968
...
if safe_files:
    try:
        ...
        assert sorted(cases) == sorted(wanted) and len(cases) == 41
