...
assert binding['profile'] == 'PUBLIC_SOURCE_RECONSTRUCTION_NONSHIPPING_APP29_FOCUSED_48'
...
assert len(set(selectors)) == len(selectors) == selection['expected_selected_methods'] == 48
assert selection['expected_total'] == 48
...
def inventory():
    ...
    assert len(current) == inventory_pin['file_count'] == 2237
...
if safe_files:
    try:
        ...
        assert sorted(cases) == sorted(wanted) and len(cases) == 48
