...
assert binding['profile'] == 'PUBLIC_SOURCE_RECONSTRUCTION_NONSHIPPING_APP29_FOCUSED_12'
...
assert len(set(selectors)) == len(selectors) == selection['expected_selected_methods'] == 12
assert selection['expected_total'] == 12
...
def inventory():
    ...
    assert len(current) == inventory_pin['file_count'] == 2490
