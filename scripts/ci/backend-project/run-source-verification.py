...
assert binding['profile'] == 'PUBLIC_SOURCE_RECONSTRUCTION_NONSHIPPING_REMAINING43_B_6'
...
if safe_files:
    try:
        ...
        ledger['exact6_and_actual_compilation'] = True
    except BaseException as error:
        partial_diagnostics()
...
ledger['remaining43_b6_pass'] = (ledger['build_exit'] == 0 and not ledger['errors'] and ledger['scratch_removed'] and
    not ledger.get('children', {}).get('forced', True) and not ledger.get('final_children', {}).get('forced', True) and
    not ledger['container_forced'] and ledger.get('exact6_and_actual_compilation', False))
emit('actual-cleanup-and-result.json', ledger)
print('Public nonshipping remaining43_b6 verification:', 'FOCUSED_PASS' if ledger['remaining43_b6_pass'] else 'NOT_ACCEPTED')
sys.exit(0 if ledger['remaining43_b6_pass'] else 1)
