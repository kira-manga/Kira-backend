...
assert binding['profile'] == 'PUBLIC_SOURCE_RECONSTRUCTION_NONSHIPPING_APP29_FOCUSED_22'
...
assert len(set(selectors)) == len(selectors) == selection['expected_selected_methods'] == 22
assert selection['expected_total'] == 22
...
def inventory():
    ...
    assert len(current) == inventory_pin['file_count'] == 2336
...
if safe_files:
    try:
        ...
        assert sorted(cases) == sorted(wanted) and len(cases) == 22
