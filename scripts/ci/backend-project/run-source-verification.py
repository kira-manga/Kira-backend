...
assert binding['profile'] == 'PUBLIC_SOURCE_RECONSTRUCTION_NONSHIPPING_FOCUSED_53'
...
assert len(set(selectors)) == len(selectors) == selection['expected_total'] == 53
...
if safe_files:
    try:
        ...
        assert sorted(cases) == sorted(wanted) and len(cases) == 53
        ...
        ledger['exact53_and_actual_compilation'] = True
    except BaseException as error:
        partial_diagnostics()
...
ledger['focused53_pass'] = (ledger['build_exit'] == 0 and not ledger['errors'] and ledger['scratch_removed'] and
    not ledger.get('children', {}).get('forced', True) and not ledger.get('final_children', {}).get('forced', True) and
    not ledger['container_forced'] and ledger.get('exact53_and_actual_compilation', False))
emit('actual-cleanup-and-result.json', ledger)
print('Public nonshipping focused53 verification:', 'FOCUSED_PASS' if ledger['focused53_pass'] else 'NOT_ACCEPTED')
sys.exit(0 if ledger['focused53_pass'] else 1)
