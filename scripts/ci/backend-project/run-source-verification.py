...
assert len(set(selectors)) == len(selectors) == selection['expected_total'] == 151
...
def inventory():
    ...
    assert len(current) == inventory_pin['file_count'] == 1833
...
common = ['--no-daemon', '--no-parallel', '--max-workers=1', '--no-build-cache', '--no-configuration-cache', '--console=plain',
    '--project-cache-dir', str(run / 'project-cache'), '-Dorg.gradle.jvmargs=-Xmx1536m -XX:MaxMetaspaceSize=512m -Djava.io.tmpdir=' + str(run / 'tmp'),
...
if safe_files:
    try:
        ...
        assert sorted(cases) == sorted(wanted) and len(cases) == 151
        ...
        ledger['exact151_and_actual_compilation'] = True
    except BaseException as error:
        partial_diagnostics()
...
ledger['source_verification_pass'] = (ledger['build_exit'] == 0 and not ledger['errors'] and ledger['scratch_removed'] and
    ...
    not ledger['container_forced'] and ledger.get('exact151_and_actual_compilation', False))
