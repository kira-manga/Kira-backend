...
assert binding['profile'] == 'PUBLIC_SOURCE_RECONSTRUCTION_NONSHIPPING_TERMINAL_A_8'
...
assert len(set(selectors)) == len(selectors) == selection['expected_total'] == 8
...
if safe_files:
    try:
        ...
        assert sorted(cases) == sorted(wanted) and len(cases) == 8
        ...
        ledger['exact8_and_actual_compilation'] = True
    except BaseException as error:
        partial_diagnostics()
...
ledger['terminal_a8_pass'] = (ledger['build_exit'] == 0 and not ledger['errors'] and ledger['scratch_removed'] and
    not ledger.get('children', {}).get('forced', True) and not ledger.get('final_children', {}).get('forced', True) and
    not ledger['container_forced'] and ledger.get('exact8_and_actual_compilation', False))
emit('actual-cleanup-and-result.json', ledger)
print('Public nonshipping terminal_a8 verification:', 'FOCUSED_PASS' if ledger['terminal_a8_pass'] else 'NOT_ACCEPTED')
sys.exit(0 if ledger['terminal_a8_pass'] else 1)
