...
assert binding['profile'] == 'PUBLIC_SOURCE_RECONSTRUCTION_NONSHIPPING_TERMINAL_RETRY_12'
...
assert len(set(selectors)) == len(selectors) == selection['expected_total'] == 12
...
def inventory():
    ...
    assert len(current) == inventory_pin['file_count'] == 1849
...
if safe_files:
    try:
        ...
        assert sorted(cases) == sorted(wanted) and len(cases) == 12
        ...
        ledger['exact12_and_actual_compilation'] = True
    except BaseException as error:
        partial_diagnostics()
...
ledger['terminal_retry12_pass'] = (ledger['build_exit'] == 0 and not ledger['errors'] and ledger['scratch_removed'] and
    not ledger.get('children', {}).get('forced', True) and not ledger.get('final_children', {}).get('forced', True) and
    not ledger['container_forced'] and ledger.get('exact12_and_actual_compilation', False))
emit('actual-cleanup-and-result.json', ledger)
print('Public nonshipping terminal_retry12 verification:', 'FOCUSED_PASS' if ledger['terminal_retry12_pass'] else 'NOT_ACCEPTED')
sys.exit(0 if ledger['terminal_retry12_pass'] else 1)
