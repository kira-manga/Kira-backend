...
assert binding['profile'] == 'PUBLIC_SOURCE_RECONSTRUCTION_NONSHIPPING_DIAGNOSTIC_2'
...
assert len(set(selectors)) == len(selectors) == selection['expected_total'] == 2
...
if safe_files:
    try:
        ...
        assert sorted(cases) == sorted(wanted) and len(cases) == 2
        ...
        ledger['exact2_and_actual_compilation'] = True
    except BaseException as error:
        partial_diagnostics()
...
ledger['diagnostic2_pass'] = (ledger['build_exit'] == 0 and not ledger['errors'] and ledger['scratch_removed'] and
    not ledger.get('children', {}).get('forced', True) and not ledger.get('final_children', {}).get('forced', True) and
    not ledger['container_forced'] and ledger.get('exact2_and_actual_compilation', False))
emit('actual-cleanup-and-result.json', ledger)
print('Public nonshipping diagnostic2 verification:', 'DIAGNOSTIC_PASS' if ledger['diagnostic2_pass'] else 'NOT_ACCEPTED')
sys.exit(0 if ledger['diagnostic2_pass'] else 1)
