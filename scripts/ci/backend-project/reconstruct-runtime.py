#!/usr/bin/env python3
"""Inert until admitted. Fresh public builds or one exact unqualified verification artifact."""
import hashlib
import json
import os
...
NEW_RUNTIME = 'NEW_UNQUALIFIED_VERIFICATION_RUNTIME'
REUSED_RUNTIME = 'REUSED_UNQUALIFIED_VERIFICATION_RUNTIME'
REUSE_FILES = {'runtime-maven.tar': 4 * 1024 * 1024, 'runtime-manifest.json': 64 * 1024,
               'runtime-provenance.json': 2 * 1024 * 1024}
REUSE_CONTROLS = ('reconstruct-runtime.py', 'runtime-binding.json', 'runtime-publication.init.gradle',
                  'runtime-repositories.init.gradle', 'normal-runtime.init.gradle', 'app29_linux_owned_processes.py')
...
def reuse_pin():
    pin = json.loads((CONTROL / 'runtime-reuse.json').read_text())
    require(set(pin) == {'schema', 'mode', 'producer'} and pin['schema'] == 1, 'Invalid runtime reuse control')
    if pin['mode'] == 'FRESH_ONLY':
        require(pin['producer'] is None, 'Fresh mode cannot carry a producer pin')
        return None
    require(pin['mode'] == 'PINNED_ARTIFACT' and isinstance(pin['producer'], dict), 'Unbound runtime reuse mode')
    producer = pin['producer']
    require(set(producer) == {'repository', 'run_id', 'run_attempt', 'head_sha', 'artifact_id', 'artifact_name',
                              'manifest', 'payload', 'provenance', 'maven_bytes'}, 'Invalid producer pin fields')
    require(producer['repository'] == 'kira-manga/Kira-backend' and type(producer['run_attempt']) is int and producer['run_attempt'] == 1 and
            all(type(producer[k]) is int and producer[k] > 0 for k in ('run_id', 'artifact_id')) and
            re.fullmatch('[0-9a-f]{40}', producer['head_sha']) and
            producer['artifact_name'] == 'verification-runtime-' + str(producer['run_id']) + '-1', 'Unbound producer identity')
    require(type(producer['maven_bytes']) is int and 0 < producer['maven_bytes'] <= 2 * 1024 * 1024, 'Invalid Maven byte pin')
    for key, name in (('manifest', 'runtime-manifest.json'), ('payload', 'runtime-maven.tar'), ('provenance', 'runtime-provenance.json')):
        row = producer[key]
        require(set(row) == {'sha256', 'bytes'} and re.fullmatch('[0-9a-f]{64}', row['sha256']) and
                type(row['bytes']) is int and 0 < row['bytes'] <= REUSE_FILES[name], 'Invalid artifact byte pin')
    return producer


def reuse_download_outputs():
    # This mode only selects a reviewed immutable artifact for the pinned download action; it never builds.
    producer = reuse_pin()
    with Path(os.environ['GITHUB_OUTPUT']).open('a') as output:
        output.write('enabled=' + ('true' if producer else 'false') + '\n')
        if producer:
            output.write('run_id=' + str(producer['run_id']) + '\nartifact_id=' + str(producer['artifact_id']) + '\n')


def expected_maven_paths():
    names = set()
    for module in ('source-contract', 'source-contract-jvm', 'source-engine', 'source-engine-jvm'):
        parent = 'me/manga/kira/source/' + module + '/'
        base = parent + '0.1.0/' + module + '-0.1.0'
        names.update(base + suffix for suffix in ('.jar', '.pom', '.module', '-sources.jar'))
        names.add(parent + 'maven-metadata-local.xml')
        if not module.endswith('-jvm'):
            names.add(base + '-kotlin-tooling-metadata.json')
    parent = 'me/manga/kira/internal/postgresql-owned-cut/'
    base = parent + '42.7.12-kira.1/postgresql-owned-cut-42.7.12-kira.1'
    for name in (base + '.jar', base + '.pom', parent + 'maven-metadata.xml'):
        names.update(name + suffix for suffix in ('', '.md5', '.sha1'))
    return names


def require_reusable_manifest(manifest):
    require(manifest['schema'] == 1 and manifest['status'] == NEW_RUNTIME and manifest['qualified'] is False and
            manifest['historicalBinaryIdentityClaimed'] is False and manifest['cleanup'] == {'engine': True, 'driver': True},
            'Only an original successful unqualified source reconstruction is reusable')
    require(manifest['sources'] == {'engineCommit': BINDING['engine']['commit'], 'pgjdbcCommit': BINDING['driver']['commit'],
                                    'recipePins': BINDING['driver']['recipe_files']}, 'Source or recipe pin changed')
    rows = manifest['mavenFiles']
    require(set(rows) == expected_maven_paths() and len(rows) == 31, 'Exactly the closed 31 Maven files required')
    for row in rows.values():
        require(set(row) == {'sha256', 'bytes'} and re.fullmatch('[0-9a-f]{64}', row['sha256']) and
                type(row['bytes']) is int and 0 < row['bytes'] <= 2 * 1024 * 1024, 'Invalid Maven file record')
    require(sum(row['bytes'] for row in rows.values()) <= 2 * 1024 * 1024, 'Maven output bound exceeded')


def runtime_inputs(java):
    return {'controls': {name: sha(CONTROL / name) for name in REUSE_CONTROLS},
            'jdk': {name: sha(java / name) for name in ('release', 'bin/java', 'lib/modules', 'lib/server/libjvm.so')}}


def require_producer_evidence(provenance, inputs):
    require(provenance['schema'] == 1 and provenance['inputs'] == inputs, 'Build controls or toolchain changed')
    proof = provenance['evidence']
    for stage in ('engine', 'driver'):
        result = proof[stage + '-result.json']
        require(result['stage'] == stage and result['pass'] is True and result['buildExit'] == 0 and result['errors'] == [] and
                result['safeToRemove'] is True and result['removed'] is True, 'Producer build/cleanup did not succeed')
        require(proof[stage + '-source-before.json'] == proof[stage + '-source-after.json'], 'Producer source drifted')
        publication = proof[stage + '-publication-result.json']
        require(publication['stage'] == stage and publication['buildFailure'] is None and
                {row['task'] for row in publication['publications']} == set(BINDING[stage]['tasks']), 'Producer publication differs')
        require(all(row['executed'] and row['didWork'] and not any(row[k] for k in ('skipped', 'upToDate', 'failure'))
                    for row in publication['publications']), 'Producer publication did not execute')
    require(proof['engine-source-before.json'] == BINDING['engine']['files'], 'Producer engine inventory differs')


def retain_runtime(runtime, evidence, manifest, inputs):
    require_reusable_manifest(manifest)
    require(files(runtime / 'maven') == manifest['mavenFiles'], 'Maven output changed before retention')
    producer = {'repository': os.environ['GITHUB_REPOSITORY'], 'run_id': int(os.environ['GITHUB_RUN_ID']),
                'run_attempt': int(os.environ['GITHUB_RUN_ATTEMPT']), 'head_sha': os.environ['GITHUB_SHA']}
    require(producer['repository'] == 'kira-manga/Kira-backend' and producer['run_id'] > 0 and producer['run_attempt'] == 1 and
            re.fullmatch('[0-9a-f]{40}', producer['head_sha']), 'Unbound artifact producer')
    producer['artifact_name'] = 'verification-runtime-' + str(producer['run_id']) + '-1'
    names = [stage + '-' + name + '.json' for stage in ('engine', 'driver')
             for name in ('result', 'source-before', 'source-after', 'publication-result')]
    provenance = {'schema': 1, 'producer': producer, 'inputs': inputs,
                  'evidence': {name: json.loads((evidence / name).read_text()) for name in names}}
    require_producer_evidence(provenance, inputs)
    bundle = evidence / 'reusable-runtime'
    bundle.mkdir(mode=0o700)
    emit(bundle / 'runtime-manifest.json', manifest)
    emit(bundle / 'runtime-provenance.json', provenance)
    with tarfile.open(bundle / 'runtime-maven.tar', 'w:', format=tarfile.USTAR_FORMAT) as archive:
        for name, row in sorted(manifest['mavenFiles'].items()):
            header = tarfile.TarInfo(name)
            header.size, header.mode = row['bytes'], 0o600
            with (runtime / 'maven' / name).open('rb') as source:
                archive.addfile(header, source)
    require(all((bundle / name).stat().st_size <= limit for name, limit in REUSE_FILES.items()), 'Retention bound exceeded')
    require(files(runtime / 'maven') == manifest['mavenFiles'], 'Maven output changed during retention')
    emit(evidence / 'retained-runtime.json', {'schema': 1, 'producer': producer, 'files': files(bundle),
                                            'maven_bytes': sum(row['bytes'] for row in manifest['mavenFiles'].values()),
                                            'qualified': False})
    with Path(os.environ['GITHUB_OUTPUT']).open('a') as output:
        output.write('retained=true\n')


def import_runtime(runtime, evidence, producer, inputs):
    bundle = evidence / 'reuse-input'
    require(bundle.is_dir() and not bundle.is_symlink(), 'Regular artifact input directory required')
    entries = []
    for entry in bundle.iterdir():
        require(len(entries) < 3 and entry.is_file() and not entry.is_symlink(), 'Only three regular artifact files allowed')
        entries.append(entry.name)
    require(set(entries) == set(REUSE_FILES), 'Exact pinned artifact filenames required')
    for key, name in (('manifest', 'runtime-manifest.json'), ('payload', 'runtime-maven.tar'), ('provenance', 'runtime-provenance.json')):
        require((bundle / name).stat().st_size == producer[key]['bytes'] and sha(bundle / name) == producer[key]['sha256'],
                'Pinned artifact size/hash mismatch; fresh fallback forbidden')
    manifest = json.loads((bundle / 'runtime-manifest.json').read_text())
    require_reusable_manifest(manifest)
    require(sum(row['bytes'] for row in manifest['mavenFiles'].values()) == producer['maven_bytes'], 'Pinned Maven size mismatch')
    provenance = json.loads((bundle / 'runtime-provenance.json').read_text())
    require(provenance['producer'] == {key: producer[key] for key in ('repository', 'run_id', 'run_attempt', 'head_sha', 'artifact_name')},
            'Producer run identity mismatch')
    require_producer_evidence(provenance, inputs)
    seen = set()
    with tarfile.open(bundle / 'runtime-maven.tar', 'r:') as archive:
        for member in archive:
            path = PurePosixPath(member.name)
            require(member.isreg() and not member.pax_headers and not path.is_absolute() and
                    all(part not in ('', '.', '..') for part in path.parts) and path.as_posix() == member.name and
                    member.name in manifest['mavenFiles'] and member.name not in seen, 'Invalid/duplicate Maven archive member')
            row = manifest['mavenFiles'][member.name]
            require(member.size == row['bytes'], 'Maven member size mismatch')
            seen.add(member.name)
            target = runtime / 'maven' / member.name
            target.parent.mkdir(mode=0o700, parents=True, exist_ok=True)
            with archive.extractfile(member) as source, target.open('xb') as output:
                shutil.copyfileobj(source, output, 65536)
            require(sha(target) == row['sha256'], 'Maven member hash mismatch')
    require(seen == set(manifest['mavenFiles']) and files(runtime / 'maven') == manifest['mavenFiles'], 'Incomplete/changed Maven import')
    emit(evidence / 'reused-producer-manifest.json', manifest)
    emit(evidence / 'reused-producer-provenance.json', provenance)
    return {**manifest, 'status': REUSED_RUNTIME, 'cleanup': {'import': True}, 'reusedFrom': producer}
...
def engine_metadata_identity(metadata, module):
    # KMP target metadata belongs to its root component, with a backlink. The
    # publication's own GAV is separately checked against its actual task and POM.
    root = module.removesuffix('-jvm')
    component = metadata['component']
    require(tuple(component[key] for key in ('group', 'module', 'version')) ==
            ('me.manga.kira.source', root, '0.1.0'), 'Gradle root component changed')
    url = '../../' + root + '/0.1.0/' + root + '-0.1.0.module'
    require(component.get('url') == (url if module.endswith('-jvm') else None), 'Gradle component backlink changed')
    for usage in ('java-api', 'java-runtime'):
        variants = [v for v in metadata['variants'] if v.get('attributes', {}).get('org.gradle.usage') == usage and
                    v.get('attributes', {}).get('org.gradle.category') == 'library' and
                    v.get('attributes', {}).get('org.jetbrains.kotlin.platform.type') == 'jvm']
        require(len(variants) == 1, 'One normal JVM library variant per usage required')
        variant = variants[0]
        if module.endswith('-jvm'):
            require('available-at' not in variant and
                    [f['url'] for f in variant.get('files', [])] == [module + '-0.1.0.jar'], 'JVM variant artifact changed')
        else:
            require(variant.get('available-at') == {'url': '../../' + root + '-jvm/0.1.0/' + root + '-jvm-0.1.0.module',
                    'group': 'me.manga.kira.source', 'module': root + '-jvm', 'version': '0.1.0'},
                    'Root JVM variant redirect changed')


def engine_artifacts(staging, evidence):
    records = []
    for module in ('source-contract', 'source-contract-jvm', 'source-engine', 'source-engine-jvm'):
        folder = staging / 'me/manga/kira/source' / module / '0.1.0'
        base = module + '-0.1.0'
        pom_coordinates(folder / (base + '.pom'), ('me.manga.kira.source', module, '0.1.0'))
        with (folder / (base + '.module')).open('rb') as stream:
            raw_metadata = stream.read(1024 * 1024 + 1)
        require(len(raw_metadata) <= 1024 * 1024, 'Bounded Gradle module metadata required')
        # Retain small fresh public metadata before interpretation, including on failure.
        (evidence / ('engine-' + module + '-metadata.json')).write_bytes(raw_metadata)
        metadata = json.loads(raw_metadata)
        engine_metadata_identity(metadata, module)
...
def run_stage(stage, runtime, evidence, recipe, java_home):
    ...
    try:
        ...
        produced = engine_artifacts(runtime / 'maven', evidence) if stage == 'engine' else driver_artifacts(directory, runtime / 'maven', env, scope, commands, resources)
...
def main():
    ...
    for path in (runtime, evidence):
        allowed = {'reuse-input'} if producer and path == evidence else set()
        require(path.stat().st_uid == os.geteuid() and path.stat().st_mode & 0o777 == 0o700 and
                {child.name for child in path.iterdir()} == allowed, 'Fresh0700 owned root or selected artifact input required')
    ...
    inputs = runtime_inputs(java)
    emit(evidence / 'jdk-inputs.json', inputs['jdk'])
    (runtime / 'runtime-started.marker').write_text('Original dependency owner started; no blind fallback deletion.\n')
    (runtime / 'maven').mkdir(mode=0o700)
    if producer:
        # Only synchronous bounded file reads/writes occur here: no source build, child or producer receipt is borrowed.
        resources = {'stage': 'reuse-import', 'started_monotonic': time.monotonic(), 'limits': BINDING['limits'], 'samples': []}
        try:
            sample_resources(runtime, resources, 'reuse-admission', admission=True)
            manifest = import_runtime(runtime, evidence, producer, inputs)
            require(files(recipe) == BINDING['driver']['recipe_files'], 'Public recipe changed during import')
            sample_resources(runtime, resources, 'reuse-import-complete')
        finally:
            require(identity(runtime) == original, 'Runtime root changed; no import cleanup receipt')
            cleanup = {'schema': 1, 'state': 'ENDED', 'rootIdentity': original, 'safeToRemove': True,
                       'mode': 'LOCAL_FILE_IMPORT_ONLY', 'stages': {}}
            emit(evidence / 'reuse-resources.json', resources)
        ...
        print('Exact reused unqualified verification runtime imported; no current source build or qualification claimed.')
        return 0
    stages, produced = {}, {}
    for stage in ('engine', 'driver'):
    ...
    manifest = {'schema': 1, 'status': NEW_RUNTIME, 'qualified': False, 'historicalBinaryIdentityClaimed': False,
    ...
    emit(runtime / 'runtime-cleanup.json', cleanup)
    emit(evidence / 'runtime-cleanup.json', cleanup)
    retain_runtime(runtime, evidence, manifest, inputs)
    print('Fresh unqualified runtime prepared; no tests or production qualification claimed.')
    return 0


if __name__ == '__main__':
    if sys.argv[1:] == ['--reuse-download-outputs']:
        reuse_download_outputs()
    else:
        require(len(sys.argv) == 1, 'Unknown reconstruction mode')
        sys.exit(main())
