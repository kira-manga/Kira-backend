#!/usr/bin/python3.12
"""INERT native selected1 successor: pre-ACK exporter loss, unchanged native trust and custody."""
import fcntl
import hashlib
...
METHODS = [
    'real native exporter loss before import acknowledgment retains unknown custody()',
]
SELECTORS = [CLASS + '.' + name.removesuffix('()') for name in METHODS]
PROFILE = 'HOSTED_NATIVE_SELECTED1_PREACK_EXPORTER_LOSS_UNQUALIFIED'
LOCK_FD = None
...
def main(cfg):
    ...
    try:
        ...
        require(len(expected) == cfg['source']['file_count'] == 2498 and
    ...
    try:
        ...
        if safe:
            try:
                ...
                if native_ok:
                    require(suite.get('tests') == '1' and all(suite.get(key) == '0' for key in ('failures', 'errors', 'skipped')), 'One real pre-ACK exporter-loss JUnit pass required')
