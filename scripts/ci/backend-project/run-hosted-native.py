#!/usr/bin/python3.12
"""INERT native selected1 successor: post-release native dump failure, unchanged native trust and custody."""
import fcntl
import hashlib
...
METHODS = [
    'real native dump timeout after exporter release refuses and keeps failed stage()',
]
SELECTORS = [CLASS + '.' + name.removesuffix('()') for name in METHODS]
PROFILE = 'HOSTED_NATIVE_SELECTED1_POST_RELEASE_FAILURE_UNQUALIFIED'
LOCK_FD = None
...
def main(cfg):
    ...
    try:
        ...
        require(len(expected) == cfg['source']['file_count'] == 2458 and
        ...
        selection = {'test_selectors': SELECTORS, 'expected_total': 1, 'expected_selected_methods': 1, 'test_counts': {CLASS: 1}}
    ...
    tasks = ['compileKotlin', 'compileTestKotlin', 'test'] + [argument for selector in SELECTORS for argument in ('--tests', selector)]
    try:
        ...
        if safe:
            try:
                ...
                if native_ok:
                    require(suite.get('tests') == '1' and all(suite.get(key) == '0' for key in ('failures', 'errors', 'skipped')), 'One real post-release failure-path JUnit pass required')
