#!/usr/bin/python3.12
"""INERT native selected2 successor: corrected address source, unchanged native trust and custody."""
import fcntl
import hashlib
...
PROFILE = 'HOSTED_NATIVE_SELECTED2_HOST_ADDRESS_UNQUALIFIED'
LOCK_FD = None
...
def main(cfg):
    ...
    try:
        ...
        selection = {'test_selectors': SELECTORS, 'expected_total': 2, 'expected_selected_methods': 2, 'test_counts': {CLASS: 2}}
    ...
    try:
        ...
        if safe:
            try:
                ...
                if native_ok:
                    require(suite.get('tests') == '2' and all(suite.get(key) == '0' for key in ('failures', 'errors', 'skipped')), 'Two real native passes required')
