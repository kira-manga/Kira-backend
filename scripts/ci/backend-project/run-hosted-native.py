#!/usr/bin/python3.12
"""INERT full native3 successor: exact JDK-parent preparation, unchanged host custody gates."""
import fcntl
import hashlib
...
PROFILE = 'HOSTED_NATIVE_CLASS3_JDK_PARENT_PREPARED_UNQUALIFIED'
LOCK_FD = None
PUBLIC_FILES = ('initial-roots.json', 'jdk-parent-permissions.json', 'host-admission.json', 'execution-roots.json', 'result.json',
...
def admit_outputs(cfg):
    ...
    try:
        ...
        stage = 'JDK_PARENT_PREPARATION'
        preparation = {key: os.environ.get('KIRA_QCAP_JDK_PARENT_' + key.upper(), '') for key in ('before', 'after', 'outcome')}
        require(all(raw == '' or re.fullmatch(r'[0-9]{1,20}( [0-9]{1,20}){3} [0-9a-f]{4,8}', raw)
                    for raw in (preparation['before'], preparation['after'])) and preparation['outcome'] in ('success', 'failure', 'cancelled', 'skipped'),
                'Bounded fixed permission receipt required', 'JDK_PARENT_RECEIPT_FORMAT')
        emit(evidence / 'jdk-parent-permissions.json', {'path': '/usr/lib/jvm', 'fields': ['device', 'inode', 'uid', 'gid', 'modeHex'],
             'recursive': False, **preparation})
        require(preparation['outcome'] == 'success' and preparation['before'] and preparation['after'],
                'Workflow JDK-parent preparation refused', 'JDK_PARENT_PREPARATION_REFUSED')
        host = cfg['host']
        stage = 'OS_RELEASE'
...
if __name__ == '__main__':
    try:
        require(len(sys.argv) == 2 and sys.argv[1] in ('--check-host-preparation', '--admit-outputs', '--run', '--publish-outputs'), 'Exact hosted mode required')
        cfg = admitted()
        if sys.argv[1] == '--check-host-preparation':
            checked(Path('/usr/lib'))  # Trusted non-symlink ancestry before the one explicit workflow chmod.
