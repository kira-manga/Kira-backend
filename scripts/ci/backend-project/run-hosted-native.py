#!/usr/bin/python3.12
"""INERT admission-only diagnostic; the CLI cannot start runtime import/builds/tests."""
import fcntl
import hashlib
...
PROFILE = 'HOSTED_NATIVE_ADMISSION_PROBE_UNQUALIFIED'
LOCK_FD = None
...
def admit_outputs(cfg):
    ...
    try:
        ...
        raise
    else:  # Admission-only probe: no artifact download, helper import or child launch follows.
        for name, original in initial.items():
            exact_owned_root(Path(name), original, observations, mismatches)
        require(not mismatches and shutil.rmtree.avoids_symlink_attacks, 'Original unstarted probe removal required')
        shutil.rmtree(work)
        require(not os.path.lexists(work), 'Original unstarted probe root remains')
        emit(evidence / 'result.json', {'profile': PROFILE, 'passed': False, 'hostAdmissionComplete': True,
             'childrenEverStarted': False, 'workRemoved': not os.path.lexists(work), 'originalRootIdentities': initial,
             'observedRootIdentities': observations, 'rootCustodyMismatches': mismatches, 'errors': []})
...
if __name__ == '__main__':
    try:
        require(len(sys.argv) == 2 and sys.argv[1] in ('--admit-outputs', '--publish-outputs'), 'Exact admission-only hosted mode required')
        cfg = admitted()
