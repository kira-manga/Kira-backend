#!/usr/bin/python3.12
"""INERT native selected1 successor: corrected pg_restore stdout fixture, unchanged native trust and custody."""
import fcntl
import hashlib
...
PROFILE = 'HOSTED_NATIVE_SELECTED1_PG_RESTORE_STDOUT_UNQUALIFIED'
LOCK_FD = None
    ...
    try:
        ...
        if safe:
            try:
                ...
                if native_ok:
                    require(suite.get('tests') == '1' and all(suite.get(key) == '0' for key in ('failures', 'errors', 'skipped')), 'One real positive native pass required')
